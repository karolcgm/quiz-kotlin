-- Adds multi-widget grading and student self-practice attempts.
-- Apply after 003_fix_rls_and_invite_flow.sql.

create table if not exists public.practice_attempts (
  id uuid primary key default gen_random_uuid(),
  student_id uuid not null references public.profiles(id) on delete cascade,
  title text not null default 'Szybki test',
...
drop policy if exists "Students read own practice attempts" on public.practice_attempts;
drop policy if exists "Students insert own practice attempts" on public.practice_attempts;
drop policy if exists "Teachers read student practice attempts" on public.practice_attempts;
drop policy if exists "Students read own practice answers" on public.practice_answers;
drop policy if exists "Students insert own practice answers" on public.practice_answers;
drop policy if exists "Teachers read student practice answers" on public.practice_answers;

create policy "Students read own practice attempts"
...
create policy "Teachers read student practice attempts"
  on public.practice_attempts for select
  using (
    exists (
      select 1
      ...
      where cm.student_id = practice_attempts.student_id
        and tc.teacher_id = auth.uid()
    )
  );

create policy "Students read own practice answers"
  on public.practice_answers for select
  using (
    exists (
      select 1
      from public.practice_attempts pa
      where pa.id = practice_answers.practice_attempt_id
        and pa.student_id = auth.uid()
    )
  );

create policy "Students insert own practice answers"
...
create policy "Teachers read student practice answers"
  on public.practice_answers for select
  using (
    exists (
      select 1
      from public.practice_attempts pa
      join public.class_members cm on cm.student_id = pa.student_id
      join public.teacher_classes tc on tc.id = cm.class_id
      where pa.id = practice_answers.practice_attempt_id
        and tc.teacher_id = auth.uid()
    )
...
grant select, insert on public.practice_answers to authenticated;
