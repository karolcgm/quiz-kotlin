-- Dodaje nagrodę za pierwsze bezbłędne ukończenie gry Łowcy Liczb.
...
begin
  ...
  case game_key
    ...
    when 'maze-67' then
      source_id := 'maze-67-perfect-first';
      reward_reason := 'Pierwsze bezbłędne ukończenie gry Labirynt 67';
      reward_title := 'Droga do 67 odnaleziona!';
    when 'factor-vault' then
      source_id := 'factor-vault-perfect-first';
      reward_reason := 'Pierwsze bezbłędne ukończenie gry Skarbiec Czynników';
      reward_title := 'Skarbiec Czynników otwarty!';
    when 'number-rangers' then
      source_id := 'number-rangers-perfect-first';
      reward_reason := 'Pierwsze bezbłędne ukończenie gry Łowcy Liczb';
      reward_title := 'Katalog Łowców Liczb jest pełny!';
    else
  ...
  if coalesce((reward_result ->> 'awarded')::boolean, false) then
    insert into public.student_reward_notifications(
      ...
      'Zdobywasz 5 punktów za pierwsze ukończenie tej gry bez żadnego błędu i bez skanera.'
    );
  end if;
