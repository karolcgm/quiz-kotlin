-- WP-002: bezpieczeństwo submit_assignment
...
create or replace function public.load_assignment_for_student_submit(target_assignment_id uuid)
...
begin
  ...
  if not public.is_active_student() then
    raise exception 'Tylko aktywny uczeń może oddawać pracę.';
  end if;

  select * into v_row from public.assignments where id = target_assignment_id;
  ...
  if v_row.status <> 'published' then
    raise exception 'Test nie jest opublikowany.';
  end if;

  if v_row.starts_at is not null and v_row.starts_at > now() then
    raise exception 'Test nie rozpoczął się jeszcze.';
  end if;

  if v_row.due_at is not null and v_row.due_at < now() then
    raise exception 'Termin oddania minął.';
  end if;

  if not exists (
    select 1 from public.assignment_students ast
    where ast.assignment_id = target_assignment_id and ast.student_id = v_student_id
  ) then
    raise exception 'Test nie jest przypisany do Ciebie.';
  end if;

  if not public.student_can_access_school(v_row.school_id) then
    raise exception 'Test nie jest dostępny w Twojej szkole.';
  end if;
  ...
  select public.is_active_student()
    and exists (
      ...
      where ast.assignment_id = target_assignment_id
        ...
        and public.student_can_access_school(a.school_id)
    );
$$;

grant execute on function public.is_active_student() to authenticated;
grant execute on function public.load_assignment_for_student_submit(uuid) to authenticated;
...
begin
  assignment_row := public.load_assignment_for_student_submit(target_assignment_id);
