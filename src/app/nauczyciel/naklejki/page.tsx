...
export default async function TeacherStickerPreviewPage({ searchParams }: { searchParams: Promise<{ collection?: string }> }) {
  ...
  return <div className="space-y-6 pb-10">
    <section className={`rounded-[2rem] p-7 text-white ${premiumCollection ? "bg-gradient-to-br from-amber-500 via-teal-700 to-slate-950" : "bg-gradient-to-br from-fuchsia-600 via-indigo-700 to-cyan-500"}`}>
      ...
      <h1 className="mt-2 text-4xl font-black">{premiumCollection ? "Tajemnicza seria" : "Prawdziwe grafiki naklejek"}</h1>
      <p className="mt-2 max-w-3xl text-white/85">{premiumCollection ? "Zawartość tej serii pozostaje ukryta także w podglądzie nauczyciela. Przyznajesz uczniowi losową niespodziankę, a jej wygląd zostaje ujawniony dopiero podczas odbioru." : "Tutaj możesz sprawdzić bitmapowe ilustracje zwykłych serii."}</p>
    </section>
    ...
    <section>
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-5">{stickers.map((sticker) => <Card key={sticker.id} className={`text-center ${premiumCollection ? "border-slate-700 bg-gradient-to-b from-slate-900 to-slate-800" : ""}`}>{premiumCollection ? <><div className="mx-auto grid h-20 w-20 place-items-center rounded-[30%] bg-slate-950 text-3xl text-amber-200">🔒</div><p className="mt-2 text-xs font-black text-white">???</p></> : <><div className="mx-auto w-fit"><AnimatedSticker stickerId={sticker.id} size="sm" /></div><p className="mt-2 text-xs font-black text-slate-800">{sticker.name}</p><p className="mt-1 text-[10px] font-black text-slate-500">#{sticker.id + 1}</p></>}</Card>)}</div>
    </section>
  </div>;
}
