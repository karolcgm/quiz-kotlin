...
export default async function TeacherMaterialsPage() {
  ...
  return <div className="space-y-7 pb-12">
    <section className="overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-slate-950 via-indigo-950 to-teal-800 p-7 text-white shadow-2xl sm:p-9">
      ...
      <p className="mt-4 max-w-3xl text-lg text-slate-200">Biblioteka łączy lekcje, ćwiczenia i gry w jednym miejscu. Pierwszy pionowy wycinek to w pełni interaktywna misja Chrupka dla działu „Liczby i działania”.</p>
    ...
    <div className="grid gap-5 xl:grid-cols-[260px_1fr]">
      <Card className="h-fit border-indigo-100"><p className="text-xs font-black uppercase tracking-[.16em] text-indigo-700">Filtry umiejętności</p><div className="mt-4 space-y-2">{["Dodawanie i odejmowanie", "Mnożenie i dzielenie", "Szacowanie wyników"].map((label, index) => <button key={label} type="button" className={`w-full rounded-xl px-3 py-3 text-left text-sm font-bold ${index === 0 ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}>{label}</button>)}</div><Link href="/nauczyciel/lekcje" className="mt-5 block text-sm font-black text-indigo-700">Przejdź do dotychczasowych lekcji →</Link></Card>
      <section>
        <div className="flex items-center justify-between gap-3">
          <div><p className="text-xs font-black uppercase tracking-[.16em] text-teal-700">1 materiał</p><h2 className="text-3xl font-black text-slate-950">Biblioteka wizualna</h2></div>
          <Link href="/nauczyciel/naklejki?collection=3" className="rounded-xl bg-amber-100 px-4 py-3 text-xs font-black text-amber-900">✨ Podgląd 20 Chrupków</Link>
        </div>
        <div className="mt-5 grid gap-5 lg:grid-cols-2">{MATERIAL_CATALOG.map((material) => (
          <Card key={material.id} className="group overflow-hidden border-cyan-100 p-0">
            ...
            <div className="p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-2 text-[11px] font-bold text-slate-600"><span className="rounded-full bg-slate-100 px-3 py-1">{material.estimatedMinutes} min</span><span className="rounded-full bg-slate-100 px-3 py-1">3 umiejętności</span><span className="rounded-full bg-slate-100 px-3 py-1">Tablet + tablica</span></div>
