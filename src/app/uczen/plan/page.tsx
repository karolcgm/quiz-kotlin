...
import { cancelStudentLessonReviewAction, getStudentLearningPlan, startStudentLessonReviewAction } from "@/lib/actions/studentLearningPlan";
...
export default async function StudentLearningPlanPage() {
  ...
  return <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
    <main className="space-y-6">
      ...
      {Array.from(grouped.entries()).map(([sectionId, sectionLessons]) => <section key={sectionId} className="space-y-3"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-wide text-indigo-600">{sectionId}</p><h2 className="text-2xl font-black text-slate-950">Przerobione tematy</h2></div><span className="rounded-full bg-indigo-50 px-3 py-1 text-sm font-black text-indigo-700">{sectionLessons.length} {sectionLessons.length === 1 ? "lekcja" : "lekcje"}</span></div>{sectionLessons.map((lesson, index) => { const percent = lesson.maxScore > 0 ? Math.round(lesson.score / lesson.maxScore * 100) : 0; return <Card key={lesson.lessonId} className="overflow-hidden border-indigo-100"><div className="grid gap-4 sm:grid-cols-[70px_1fr_auto] sm:items-center"><div className="grid h-16 w-16 place-items-center rounded-2xl bg-gradient-to-br from-cyan-300 to-indigo-600 text-3xl font-black text-white">{index + 1}</div><div><div className="flex flex-wrap items-center gap-2"><span className="text-xs font-black uppercase text-indigo-600">{lesson.topicId}</span>{lesson.completedAttempts > 0 ? <span className="rounded-full bg-emerald-100 px-2 py-1 text-[10px] font-black text-emerald-800">zaliczono {lesson.completedAttempts}×</span> : null}</div><h3 className="mt-1 text-xl font-black text-slate-950">{lesson.lessonTitle}</h3>{lesson.textbookPage ? <p className="mt-2 text-sm font-bold text-indigo-700">Podręcznik: strona {lesson.textbookPage}{lesson.coveredExercises.length > 0 ? ` · zadania ${lesson.coveredExercises.join(", ")}` : ""}</p> : null}<div className="mt-3 h-3 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-gradient-to-r from-amber-400 to-emerald-500" style={{ width: `${percent}%` }} /></div><p className="mt-1 text-xs font-bold text-slate-500">Najlepszy wynik: {lesson.score}/{lesson.maxScore} pkt</p></div><div>{lesson.inProgressReviewId ? <div className="flex flex-wrap gap-2"><Link href={`/uczen/plan/powtorka/${lesson.inProgressReviewId}`} className="inline-flex min-h-12 items-center rounded-xl bg-amber-500 px-5 text-sm font-black text-white">Wróć do zaliczenia</Link><form action={cancelStudentLessonReviewAction}><input type="hidden" name="reviewId" value={lesson.inProgressReviewId}/><button className="min-h-12 rounded-xl border border-rose-200 bg-rose-50 px-5 text-sm font-black text-rose-800" title="Zakończ to podejście bez zapisywania wyniku">Zakończ</button></form></div> : <form action={startStudentLessonReviewAction}><input type="hidden" name="lessonId" value={lesson.lessonId}/><button className="min-h-12 rounded-xl bg-indigo-600 px-5 text-sm font-black text-white">{lesson.completedAttempts > 0 ? "Zalicz ponownie" : "Zalicz"}</button></form>}</div></div></Card>; })}</section>)}
    </main>
