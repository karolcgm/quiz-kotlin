...
export default async function StudentAgileGameDetail({ params }: { params: Promise<{ gameId: string }> }) {
  ...
  if (game.template_id === "zoo-sprint" && me && game.status === "active") { const { data: sprint } = await supabase.from("agile_zoo_sprints").select("id,event_id").eq("session_id", gameId).eq("status", "planning").order("sprint_number", { ascending: false }).limit(1).maybeSingle<{ id: string; event_id:string|null }>(); const { data: state } = await supabase.from("agile_zoo_team_state").select("budget").eq("session_id", gameId).eq("team_id", me.team_id).maybeSingle<{ budget: number }>(); const { data: zooChoices } = sprint ? await supabase.from("agile_zoo_task_choices").select("task_id").eq("sprint_id", sprint.id).eq("team_id", me.team_id) : { data: [] }; const {data:previousSprints}=sprint?await supabase.from("agile_zoo_sprints").select("id").eq("session_id",gameId).neq("id",sprint.id):{data:[]};const previousIds=(previousSprints??[]).map(row=>row.id);const {data:usedChoices}=previousIds.length?await supabase.from("agile_zoo_task_choices").select("task_id").eq("team_id",me.team_id).in("sprint_id",previousIds):{data:[]}; const selected=(zooChoices??[]).map(row=>row.task_id); const used=(usedChoices??[]).map(row=>row.task_id);const spent=selected.reduce((sum,id)=>sum+(ZOO_TASK_BY_ID.get(id)?.cost??0),0); if(sprint) zooBoard=<StudentZooSprintBoard sprintId={sprint.id} eventId={sprint.event_id} roles={me.roles??[]} selected={selected} used={used} budget={state?.budget??50} spent={spent}/>; }
