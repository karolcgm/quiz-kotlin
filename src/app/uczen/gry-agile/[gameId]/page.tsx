...
export default async function StudentAgileGameDetail({ params }: { params: Promise<{ gameId: string }> }) {
  ...
  if (game.template_id === "zoo-sprint" && me && game.status === "active") { const { data: sprint } = await supabase.from("agile_zoo_sprints").select("id").eq("session_id", gameId).eq("status", "planning").order("sprint_number", { ascending: false }).limit(1).maybeSingle<{ id: string }>(); const { data: state } = await supabase.from("agile_zoo_team_state").select("budget").eq("session_id", gameId).eq("team_id", me.team_id).maybeSingle<{ budget: number }>(); const { data: zooChoices } = sprint ? await supabase.from("agile_zoo_task_choices").select("task_id").eq("sprint_id", sprint.id).eq("team_id", me.team_id) : { data: [] }; const selected=(zooChoices??[]).map(row=>row.task_id); const spent=selected.reduce((sum,id)=>sum+(ZOO_TASK_BY_ID.get(id)?.cost??0),0); if(sprint) zooBoard=<StudentZooSprintBoard sprintId={sprint.id} roles={me.roles??[]} selected={selected} budget={state?.budget??50} spent={spent}/>; }
