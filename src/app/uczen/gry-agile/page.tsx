...
export default async function StudentAgileGamesPage() {
  ...
  const { data: games } = classIds.length ? await supabase.from("agile_game_sessions").select("id, title, template_id, status, sprint_number, teacher_classes(name, group_name)").in("class_id", classIds).in("status", ["lobby", "active"]).order("created_at", { ascending: false }) : { data: [] };
