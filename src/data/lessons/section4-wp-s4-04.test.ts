...
describe("WP-S4-04 — pakiet L1 kątów przyległych i wierzchołkowych", () => {
  ...
  it("prowadzi przez wszystkie wymagane historie do dokładnie jednej Oceny umiejętności", () => {
  ...
  it("wiąże support/core/challenge z końcową oceną i osobną punktacją uzasadnienia", () => {
    const independent = m544SkrzyzowanieProstychV1.stages.at(-2)!;
    expect(independent.print?.items?.map((item) => item.id)).toEqual([
      "independent-pairs-support",
      "independent-calculation-core",
      "independent-justification-challenge",
    ]);
    expect(independent.print?.items?.map((item) => item.maxScore)).toEqual([1, 2, 3]);
    expect(independent.print?.items?.at(-1)?.skillIds).toEqual(["M5-4.4-angle-pairs-properties", "M5-4.4-angle-calculations"]);
