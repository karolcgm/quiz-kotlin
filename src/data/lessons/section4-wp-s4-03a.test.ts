...
describe("WP-S4-03A — pakiet L1 pomiaru", () => {
  it("ma oficjalny trace-0, nazwę i wymaganie VIII.2 dotyczące pomiaru", () => {
    ...
    expect(refs.some((reference) => reference.startsWith("VIII.3 —"))).toBe(false);
  ...
  it("prowadzi przez 10 pomiarów do jednej Oceny umiejętności bez dodatkowych rodzajów zadań", () => {
    ...
    expect(m543KatomierzEkranowyV1.stages.reduce((sum, stage) => sum + stage.estimatedMinutes, 0)).toBe(45);
  });
  ...
  it("wiąże 10 pomiarów z końcową oceną", () => {
    const independent = m543KatomierzEkranowyV1.stages.at(-2)!;
    expect(independent.print?.items).toHaveLength(10);
    expect(independent.print?.items?.map((item) => item.id)).toEqual(Array.from({ length: 10 }, (_, index) => `setup-angle-measurement-${index + 1}`));
