...
describe("WP-S4-03B — pakiet L2 rysowania", () => {
  ...
  it("prowadzi przez 65°, warianty, anonimową kontrolę i samodzielny dowód do jednej oceny", () => {
    ...
    expect(titles).toEqual(expect.arrayContaining(["Etapy rysowania kąta", "Rysowanie kąta 65° krok po kroku", "Kolejność rysowania kąta", "Rysowanie kątów w różnych położeniach", "Sprawdzanie narysowanego kąta", "Samodzielne rysowanie kąta"]));
    expect(titles).not.toContain("Samodzielny pomiar");
    expect(titles.at(-2)).toBe("Samodzielne rysowanie kąta");
