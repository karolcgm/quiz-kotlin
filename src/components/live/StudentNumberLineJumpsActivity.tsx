...
export function StudentNumberLineJumpsActivity({ sessionId, stageId, station, question, submitted, questionNumber, questionCount, onRefresh }: Props) {
  ...
  const handleResult = useCallback((correct: boolean | null, answer?: string) => {
    setFeedback(null);
    setResult(correct === null ? null : { correct, answer: answer ?? "" });
  }, []);
  ...
  return <div className="space-y-4">
    ...
    <button
      type="button"
      disabled={pending || !result || feedback !== null}
      onClick={() => startTransition(async () => {
        ...
        if (!response.ok) {
          setError(response.error ?? "Nie udało się wysłać odpowiedzi.");
          return;
        }
    ...
    >
      {pending ? "Sprawdzanie…" : result ? `Zatwierdź ${questionNumber}/${questionCount}` : "Najpierw sprawdź uzupełnioną oś"}
    </button>
    {feedback !== null ? (
      <div className={`rounded-2xl p-4 text-center font-black ${feedback ? "bg-emerald-100 text-emerald-950" : "bg-amber-100 text-amber-950"}`} role="status">
        <p>{feedback ? "Dobrze! Punkt został zapisany." : "Tym razem nie. Zadanie zostało zapisane bez punktu."}</p>
        <button type="button" onClick={() => void onRefresh()} className="mt-3 min-h-12 rounded-xl bg-slate-950 px-5 text-white">
          {questionNumber < questionCount ? "Dalej" : "Zakończ serię"}
        </button>
      </div>
    ) : null}
  </div>;
