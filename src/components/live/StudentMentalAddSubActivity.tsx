...
export function StudentMentalAddSubActivity({ sessionId, stageId, station, question, submitted, questionNumber, questionCount, onRefresh }: Props) {
  ...
  return <div className="space-y-4">
    <MentalAddSubLessonModel seed={station} taskSeed={question.seed} questionNumber={questionNumber} questionCount={questionCount} onResultChange={handleResult} />
    ...
    <button type="button" disabled={pending || !result || feedback !== null} onClick={() => startTransition(async () => {
      ...
      {pending ? "Sprawdzanie…" : result ? `Zatwierdź ${questionNumber}/${questionCount}` : "Uzupełnij wynik"}
    </button>
