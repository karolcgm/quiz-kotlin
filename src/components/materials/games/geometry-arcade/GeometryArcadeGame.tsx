...
export function GeometryArcadeGame({gameKey,rewardEnabled=true}:{gameKey:GeometryGameKey;rewardEnabled?:boolean}) {
  ...
  const next=()=>{const finalScore=score+(selected===current.correct&&!answered?1:0);if(round===config.rounds.length-1){setDone(true);if(!rewardEnabled){setSaved("Podgląd nauczyciela — wynik nie jest zapisywany.");return}void claimGeometryGameScoreAction(gameKey,finalScore,config.rounds.length).then(r=>setSaved(r.error??(r.awardedPoints>0?`Zdobywasz ${r.awardedPoints} pkt!`:"Ten wynik był już zapisany — punktów nie dublujemy.")));return}setRound(v=>v+1);setSelected(null);setAnswered(false)};
