...
import { SpaceRouteMap } from "@/components/materials/games/space-courier/SpaceRouteMap";
...
export function SpaceCourierGame({ rewardEnabled = false }: { rewardEnabled?: boolean }) {
  ...
  return <section className="overflow-hidden rounded-[2rem] border border-violet-200 bg-slate-950 shadow-2xl" aria-label="Gra Kosmiczny Kurier">
    <div className="relative aspect-[16/9] min-h-[780px] overflow-hidden bg-indigo-950 sm:min-h-[680px] lg:min-h-[600px]">
      ...
      {status === "playing" && round ? <div className="absolute inset-0 z-10 overflow-y-auto px-[4%] pb-4 pt-24 sm:pt-28">
        <div className="w-full rounded-2xl border border-white/30 bg-slate-950/72 p-3 text-center text-white shadow-xl backdrop-blur-md"><p className="text-[10px] font-black uppercase tracking-[.16em] text-cyan-200">Dostawa {roundIndex + 1}/{rounds.length} · wybierz krok {selected.length + 1}</p><h2 className="mt-1 font-mono text-xl font-black sm:text-3xl">{round.expression}</h2><p className="mt-2 text-sm font-bold text-amber-200" aria-live="polite">{stepInstruction}</p></div>

        <div className="mt-4 grid items-start gap-4 lg:grid-cols-[minmax(0,3fr)_minmax(290px,2fr)]">
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            {round.steps.map((step, index) => {
              const active = selected.includes(step.id);
              return <button key={step.id} type="button" onClick={() => choose(step.id)} disabled={active || feedback === "correct"} className={`space-route-node relative min-h-20 rounded-2xl border-2 p-3 text-sm font-black shadow-xl backdrop-blur-md transition hover:-translate-y-1 sm:min-h-24 sm:text-lg ${active ? "border-emerald-200 bg-emerald-300 text-emerald-950 shadow-[0_0_28px_rgba(52,211,153,.7)]" : "border-cyan-200/60 bg-indigo-950/82 text-white hover:border-cyan-100 hover:bg-indigo-900/90"}`} style={{ animationDelay: `${index * 100}ms` }}><span className="mb-1 block text-[9px] uppercase tracking-[.18em] opacity-70">Karta obliczenia</span>{step.label}</button>;
            })}
          </div>
          <SpaceRouteMap completedSteps={selected.length} />
        </div>

        <div className="mt-3 min-h-12 w-full" aria-live="polite">{feedback === "wrong" ? <p className="rounded-xl bg-rose-50/95 p-3 text-center text-sm font-bold text-rose-900">To była pułapka. Zacznij tę trasę od pierwszego kroku. {round.hint}</p> : feedback === "correct" ? <p className="rounded-xl bg-emerald-50/95 p-3 text-center text-sm font-black text-emerald-900">Trasa gotowa! Przesyłka dotarła z wynikiem {round.result}.</p> : null}</div>
      </div> : null}
