...
type Team = { id:string; name:string; color:string; visitors:number; budget:number; crises:string[]; selected:number[]; used:number[] };
...
export function EngineSprintBoard({ sprintId, sprintNumber, status, templateId, eventId, teams }: { sessionId:string; sprintId:string; sprintNumber:number; status:string; templateId:string; eventId:string|null; teams:Team[] }) {
  ...
  return <section className="overflow-hidden rounded-[2rem] bg-slate-950 text-white shadow-xl"><div className="p-6 sm:p-8" style={{ backgroundImage:`linear-gradient(rgb(2 6 23/.84),rgb(2 6 23/.95)),url('${game.image}')`, backgroundSize:"cover" }}>
    ...
    {crisis ? <div className="mt-5 rounded-2xl border border-rose-300/60 bg-rose-950/85 p-5"><p className="text-xs font-black uppercase tracking-widest text-rose-200">Kryzys sprintu</p><h2 className="mt-1 text-2xl font-black">{crisis.title}</h2><p className="mt-2 text-rose-50">{crisis.body}</p>{status !== "planning" ? <p className="mt-3 font-bold text-amber-100">{crisis.fixes.some(id => selected.includes(id)) ? "Zespół zareagował na kryzys w tym sprincie." : crisis.consequence}</p> : null}</div> : null}
