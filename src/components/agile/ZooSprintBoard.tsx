...
export type ZooBoardTeam = { id:string; name:string; color:string; visitors:number; budget:number; crises:string[]; selected:number[]; used:number[]; history?:Array<{sprint:number;visitors:number;budget:number}>; story?:Array<{sprint:number;body:string}> };
...
export function ZooSprintBoard({ sprintId, sprintNumber, status, eventId, teams }: { sprintId:string; sprintNumber:number; status:string; eventId?:string|null; teams:ZooBoardTeam[] }) {
  ...
  const finish = () => startTransition(async () => { const result=await finishZooGameAction(sprintId); if(!result.ok)setMessage(result.error); else router.push("/nauczyciel/gry-klasowe"); });
  const hasCrisis = teams.some(item => item.crises.length > 0);
  const sprintEvent = eventId ? ZOO_EVENT_BY_ID.get(eventId) : undefined;
  const sprintEventResolved = Boolean(sprintEvent?.isSetup || sprintEvent?.requiredTaskIds.some(id=>team.selected.includes(id)));

  return <><AgileLiveRefresh active={status === "planning"} intervalMs={2000}/><section ref={boardRef} className={`overflow-auto bg-slate-950 text-white ${fullscreen?"h-screen w-screen rounded-none":"rounded-[2rem]"}`}>
    <div className="p-6" style={{backgroundImage:"linear-gradient(rgb(2 6 23/.7),rgb(2 6 23/.8)),url('/agile-games/zoo-sprint.png')",backgroundSize:"cover"}}>
      ...
      {sprintEvent?<div className="mb-4 rounded-2xl border-2 border-rose-300 bg-rose-950/85 p-5 shadow-xl"><p className="text-sm font-black uppercase tracking-widest text-rose-200">{sprintEvent.isSetup?"📋 Sytuacja startowa":"⚠ Kryzys sprintu"}</p><h2 className="mt-1 text-2xl font-black">{sprintEvent.title}</h2><p className="mt-2 text-rose-50">{sprintEvent.body}</p>{status!=="planning"&&!sprintEvent.isSetup?<p className={`mt-3 rounded-xl p-3 font-bold ${sprintEventResolved?"bg-emerald-400/20 text-emerald-100":"bg-rose-400/20 text-rose-100"}`}><b>{sprintEventResolved?"Kryzys opanowany: ":"Kryzys nierozwiązany: "}</b>{sprintEventResolved?sprintEvent.success:sprintEvent.failure}</p>:status!=="planning"?<p className="mt-3 rounded-xl bg-slate-950/40 p-3 font-bold text-amber-100">Omówcie, które z czterech pilnych problemów udało się zabezpieczyć, a które eskalowały.</p>:<p className="mt-3 font-bold text-amber-200">To jedyne cztery problemy, które mogą eskalować na starcie. Skutki decyzji są ukryte.</p>}</div>:null}
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_300px]">
          <div><h2 className="text-3xl font-black">{activeRole} · 15 decyzji</h2><p className="mt-2 text-cyan-100">Wybierz drużynę po prawej, potem zaznacz jej decyzje. Skutki pozostają ukryte aż do zakończenia sprintu.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{ZOO_TASKS.filter(task=>task.role===activeRole).map(task=>{const chosen=team.selected.includes(task.id);const alreadyUsed=team.used.includes(task.id);const blocked=!chosen&&(alreadyUsed||spent+task.cost>team.budget||team.selected.some(id=>task.blocks?.includes(id)||ZOO_TASK_BY_ID.get(id)?.blocks?.includes(task.id)));const markers=teams.map((item,index)=>({item,index})).filter(({item})=>selectionFor(item).includes(task.id));return <button key={task.id} disabled={pending||status!=="planning"||blocked} onClick={()=>choose(task.id)} className={`relative rounded-2xl border p-4 pt-8 text-left ${chosen?"border-emerald-300 bg-emerald-400/25":alreadyUsed?"border-slate-700 bg-slate-900/60 text-slate-400":"border-white/20 bg-white/10"} disabled:opacity-35`}><span className="absolute left-3 top-2 flex gap-1">{markers.map(({item,index})=><span key={item.id} className="grid size-6 place-items-center rounded-md text-xs font-black text-white" style={{background:item.color}}>{index+1}</span>)}</span><b>#{task.id}</b><span className="mt-1 block font-black">{task.title}</span><span className="mt-2 block text-sm">{alreadyUsed?"Wykorzystane w poprzednim sprincie":`Koszt: ${task.cost} pkt`}</span>{task.budgetImpact!==0?<span className={`mt-1 block text-xs font-black ${task.budgetImpact>0?"text-emerald-200":"text-amber-200"}`}>Budżet po sprincie: {task.budgetImpact>0?"+":""}{task.budgetImpact} pkt</span>:null}{task.blocks?<span className="mt-1 block text-xs text-amber-200">Wariant alternatywny — blokuje inne rozwiązanie.</span>:null}{status!=="planning"&&chosen?<span className="mt-3 block rounded-xl bg-slate-950/70 p-3 text-sm leading-relaxed text-cyan-50"><b>Skutek decyzji:</b> {task.outcome}<strong className={`mt-2 block ${task.visitors>=0?"text-emerald-200":"text-rose-200"}`}>{task.visitors>=0?"+":""}{task.visitors} tys. odwiedzających</strong><strong className={`mt-1 block ${task.budgetImpact>=0?"text-emerald-200":"text-amber-200"}`}>Budżet: {task.budgetImpact>=0?"+":""}{task.budgetImpact} pkt</strong></span>:null}</button>})}</div></div>
      ...
      <div className="mt-5 flex flex-wrap gap-3">{status==="planning"?<button onClick={reveal} disabled={pending} className="rounded-xl bg-emerald-400 px-5 py-3 font-black text-emerald-950 disabled:bg-slate-500">GM: odkryj skutki sprintu</button>:status==="revealed"&&sprintNumber<6?<button onClick={advance} disabled={pending} className="rounded-xl bg-white px-5 py-3 font-black text-indigo-700">Rozpocznij sprint {sprintNumber+1}</button>:status==="revealed"?<button onClick={finish} disabled={pending} className="rounded-xl bg-rose-500 px-5 py-3 font-black text-white disabled:bg-slate-500">Zakończ grę i usuń sesję</button>:<span className="rounded-xl bg-amber-300 px-5 py-3 font-black text-slate-950">Gra zakończona</span>}{message?<p className="rounded-xl bg-rose-500/25 px-4 py-3 font-bold">{message}</p>:null}</div>
    ...
    {status!=="planning"?<div className="grid min-h-64 place-items-center bg-cover bg-center p-8 text-center" style={{backgroundImage:`linear-gradient(rgb(2 6 23/.4),rgb(2 6 23/.62)),url('${hasCrisis?"/agile-games/zoo-crisis.png":"/agile-games/zoo-success.png"}')`}}><div><p className="text-5xl">{hasCrisis?"⚠️":"🏆"}</p><h2 className="mt-3 text-3xl font-black">{hasCrisis?"Zoo mierzy się z konsekwencjami":"Udany sprint — zoo rośnie"}</h2><p className="mt-2 text-lg text-white/90">Omówcie wynik przed rozpoczęciem kolejnego sprintu.</p></div></div>:null}
  </section></>;
}
