...
export function SelfPacedLessonPlayer({ initialReview }: { initialReview: StudentLessonReviewView }) {
  ...
  return <div ref={playerRef} className="self-paced-lesson grid gap-5 lg:grid-cols-[230px_minmax(0,1fr)_220px]" data-fullscreen={isFullscreen || undefined}>
    ...
    <aside className="order-3"><Card className="border-0 bg-slate-950 text-white lg:sticky lg:top-4"><p className="text-xs font-black uppercase tracking-[.18em] text-cyan-300">Twoje punkty</p><p className="mt-3 text-5xl font-black tabular-nums">{score}<span className="text-2xl text-slate-400">/{initialReview.maxScore}</span></p><div className="mt-4 h-4 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-yellow-300 to-emerald-400" style={{ width: `${progressPercent}%` }} /></div><p className="mt-2 text-xs text-slate-400">Odpowiedzi: {answeredCount}/{initialReview.maxScore}</p><div className="mt-5 rounded-2xl bg-white/10 p-3 text-center"><div className="text-4xl">⭐</div><p className="mt-2 text-sm font-black">Popraw najlepszy wynik, aby uzupełnić pulę punktów. Za 100% tematu dostaniesz jedną naklejkę.</p></div><Link href="/uczen/plan" className="mt-4 inline-flex min-h-11 w-full items-center justify-center rounded-xl bg-white/10 text-sm font-black">Zapisz i wyjdź</Link></Card></aside>
  </div>;
}
