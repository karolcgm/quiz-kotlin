...
export function SelfPacedLessonPlayer({
  ...
  return <div className={`self-paced-lesson lesson-theme-${themeId} grid gap-5 lg:grid-cols-[230px_minmax(0,1fr)_220px]`} data-fullscreen={isFullscreen || undefined}>
    ...
    <main className="order-1 min-w-0 space-y-4 lg:order-2"><header className="rounded-[2rem] bg-gradient-to-r from-indigo-600 to-fuchsia-600 p-5 text-white"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.18em] text-indigo-100">{initialReview.stageSnapshot.topicId} · podejście {initialReview.attemptNumber}</p><h1 className="mt-1 text-2xl font-black">{initialReview.stageSnapshot.title}</h1><p className="mt-2 text-sm text-indigo-100">Wybieraj slajdy z lewej strony albo przechodź przyciskiem „Dalej”.</p></div><button type="button" onClick={() => void toggleFullscreen()} className="inline-flex min-h-11 items-center rounded-xl bg-white/15 px-4 text-sm font-black text-white ring-1 ring-white/30 hover:bg-white/25">⛶ Pełny ekran slajdu</button></div>
      <div className="mt-4 flex flex-wrap items-center gap-2" aria-label="Motyw slajdów"><span className="mr-1 text-xs font-black uppercase tracking-wide text-white/80">Motyw:</span>{REWARD_THEMES.map((theme) => { const unlocked = totalPoints >= theme.points; return <button key={theme.id} type="button" disabled={!unlocked || pending} title={unlocked ? theme.name : `Odblokujesz za ${theme.points} pkt`} onClick={() => startTransition(async () => { const previous = themeId; setThemeId(theme.id); const response = await selectStudentLessonThemeAction(theme.id); if (!response.ok) { setThemeId(previous); setError(response.error); } })} className={`min-h-10 rounded-full px-3 text-xs font-black ring-1 ring-white/30 ${themeId === theme.id ? "bg-white text-indigo-700" : "bg-white/10 text-white"} disabled:cursor-not-allowed disabled:opacity-40`}>{theme.emoji} {theme.name}</button>; })}</div></header>
    ...
    <aside className="order-3"><Card className="border-0 bg-slate-950 text-white lg:sticky lg:top-4"><p className="text-xs font-black uppercase tracking-[.18em] text-cyan-300">Wynik tematu</p><p className="mt-3 text-5xl font-black tabular-nums">{score}<span className="text-2xl text-slate-400">/{initialReview.maxScore}</span></p><div className="mt-4 h-4 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-yellow-300 to-emerald-400" style={{ width: `${progressPercent}%` }} /></div><p className="mt-2 text-xs text-slate-400">Odpowiedzi: {answeredCount}/{initialReview.maxScore}</p><div className="mt-5 rounded-2xl bg-white/10 p-3 text-center"><div className="text-4xl">⭐</div><p className="mt-2 text-sm font-black">Pula tematu to 20 pkt. Od 50% wyniku dostajesz punkty, a poprawa dopłaca tylko brakującą różnicę. Za 100% otrzymasz jedną naklejkę.</p></div>{allAnswered ? <button type="button" onClick={() => { setStageIndex(stages.length - 1); window.setTimeout(() => presentationRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 0); }} className="mt-4 inline-flex min-h-12 w-full items-center justify-center rounded-xl bg-emerald-500 px-3 text-sm font-black text-slate-950">Zakończ lekcję →</button> : <Link href="/uczen/plan" className="mt-4 inline-flex min-h-11 w-full items-center justify-center rounded-xl bg-white/10 text-sm font-black">Zapisz i wyjdź</Link>}</Card></aside>
  </div>;
}
