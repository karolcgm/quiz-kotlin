...
function IntroTask({ readOnly, onResultChange }: Pick<Props, "readOnly" | "onResultChange">) {
  ...
  const arrangements = [
    { label: "1 × 28", valid: true },
    { label: "2 × 14", valid: true },
    { label: "4 × 7", valid: true },
    { label: "3 × 9", valid: false },
    { label: "5 × 6", valid: false },
  ];
  ...
  const check = () => {
    ...
    setChecked(correct);
    onResultChange?.(correct, `Dzielniki 28: ${divisors}; układy: ${[...selected].map((index) => arrangements[index]?.label).join(", ")}`);
  };
  return <div className="space-y-5">
    <article className="grid overflow-hidden rounded-3xl bg-white text-slate-950 shadow-xl lg:grid-cols-[1fr_1fr]">
      <div className="p-5 sm:p-7"><p className="text-xs font-black uppercase tracking-[.18em] text-emerald-700">1. Dzielniki w życiu</p><h4 className="mt-2 text-2xl font-black">Odznaki do równych rzędów</h4><p className="mt-3 leading-relaxed text-slate-700">Chrupek ma <b>28 odznak</b>. Chce układać je w równe rzędy bez odkładania żadnej odznaki. Liczba elementów w jednym rzędzie musi więc być <b>dzielnikiem 28</b>.</p><p className="mt-4 rounded-2xl bg-emerald-50 p-4 font-bold text-emerald-950"><b>Dzielnik</b> to liczba, przez którą można podzielić daną liczbę bez reszty.</p><label className="mt-4 block font-black">Wypisz rosnąco wszystkie dzielniki 28<input aria-label="Wszystkie dzielniki liczby 28" disabled={readOnly} value={divisors} onChange={(event) => { setDivisors(event.target.value); reset(); }} placeholder="1, 2, …" className="mt-2 min-h-14 w-full rounded-xl border-2 border-emerald-200 bg-emerald-50 px-4 text-lg font-bold outline-none focus:border-emerald-600" /></label></div>
      <div className="relative min-h-80 bg-emerald-50 lg:min-h-full"><Image src="/lessons/illustrations/number-properties/chrupek-divisors-badges-v1.webp" alt="Chrupek układa kolorowe okrągłe odznaki w równych rzędach" fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover object-left" /></div>
    </article>
...
function OrbitTask({ taskIndex, readOnly, onResultChange }: { taskIndex: number } & Pick<Props, "readOnly" | "onResultChange">) {
  ...
  const check = () => {
    const expected = task.candidates.filter((value) => task.center % value === 0);
...
function GcdCalculationTask({ taskIndex, readOnly, onResultChange }: { taskIndex: number } & Pick<Props, "readOnly" | "onResultChange">) {
  ...
  return <article className="rounded-[2rem] bg-gradient-to-br from-slate-950 via-emerald-950 to-indigo-950 p-5 text-white shadow-2xl sm:p-8"><p className="text-sm font-black text-emerald-200">Wypisz dzielniki rosnąco. Największy wspólny zakreśl w myślach.</p><h4 className="mt-1 text-3xl font-black">Oblicz NWD({task.a}, {task.b})</h4><div className="mt-6 space-y-4 rounded-3xl bg-white p-5 text-slate-950"><label className="block font-black">Dzielniki {task.a}<input aria-label={`Dzielniki liczby ${task.a}`} disabled={readOnly} value={lineA} onChange={(event) => change(setLineA, event.target.value)} placeholder="1, 2, …" className="mt-2 min-h-14 w-full rounded-xl border-2 border-emerald-200 bg-emerald-50 px-4 text-lg font-bold outline-none focus:border-emerald-600" /></label><label className="block font-black">Dzielniki {task.b}<input aria-label={`Dzielniki liczby ${task.b}`} disabled={readOnly} value={lineB} onChange={(event) => change(setLineB, event.target.value)} placeholder="1, 2, …" className="mt-2 min-h-14 w-full rounded-xl border-2 border-indigo-200 bg-indigo-50 px-4 text-lg font-bold outline-none focus:border-indigo-600" /></label><label className="flex flex-wrap items-center gap-3 text-xl font-black">NWD({task.a}, {task.b}) = <input aria-label={`NWD liczb ${task.a} i ${task.b}`} inputMode="numeric" disabled={readOnly} value={result} onChange={(event) => change(setResult, event.target.value.replace(/\D/g, ""))} className="min-h-14 w-32 rounded-xl border-2 border-amber-300 bg-amber-50 px-4 text-center text-2xl font-black" /></label></div><button type="button" disabled={readOnly || !lineA.trim() || !lineB.trim() || !result} onClick={check} className="mt-5 min-h-14 w-full rounded-2xl bg-emerald-300 px-5 text-lg font-black text-emerald-950 disabled:opacity-35">Sprawdź NWD</button><Feedback correct={checked} /></article>;
}
