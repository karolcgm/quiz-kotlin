...
function WrittenAnswerGrid({ a, b, operation, readOnly, onResultChange }: { a: number; b: number; operation: "+" | "−"; readOnly: boolean; onResultChange?: (correct: boolean | null, answer?: string) => void }) {
  ...
  const change = (digit: string) => {
    ...
    const next = current.map((value, index) => index === active.column
    ...
    if (active.row === "result") {
      ...
      onResultChange?.(next.every(Boolean) ? Number(answer) === expected : null, answer || undefined);
    } else {
      setCarries(next);
    }
