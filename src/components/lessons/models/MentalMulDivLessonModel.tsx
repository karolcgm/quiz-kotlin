...
function Frame({ title, instruction, accent, children }: { title: string; instruction: string; accent: string; children: ReactNode }) {
  ...
  if (brightFrame) return <section className={`relative isolate overflow-hidden rounded-[2rem] bg-gradient-to-br ${accent} p-4 text-white shadow-2xl sm:p-7`}><header className="flex items-start justify-between gap-3"><div><p className="text-xs font-black tracking-[.2em] text-cyan-100">LICZBY I DZIAŁANIA · TEMAT 3</p><h3 className="mt-1 text-3xl font-black sm:text-5xl">{title}</h3><p className="mt-2 max-w-3xl text-sm text-white/90 sm:text-lg">{instruction}</p></div>{progress ? <span className="shrink-0 rounded-2xl bg-cyan-100 px-4 py-2 text-sm font-black text-slate-950">Zadanie {progress.number}/{progress.count}</span> : null}</header><div className="mt-6">{children}</div></section>;
...
function Ready({ correct, answer }: { correct: boolean; answer: string }) { const report = useContext(ReporterContext); useEffect(() => { report?.(correct, answer); return () => report?.(null); }, [answer, correct, report]); return correct ? <p role="status" className="mt-5 rounded-2xl bg-emerald-100 px-4 py-3 text-center font-bold text-emerald-950">Dobrze — odpowiedź jest poprawna.</p> : <p role="status" className="mt-5 rounded-2xl bg-rose-100 px-4 py-3 text-center font-bold text-rose-950">Sprawdź wynik i spróbuj ponownie.</p>; }
...
function DigitAnswer({ expected, readOnly }: { expected: number; readOnly: boolean }) {
  const [digits, setDigits] = useState([0, 0, 0, 0, 0]); const [touched, setTouched] = useState(false); const [submitted, setSubmitted] = useState(false); const value = digits[0]! * 10000 + digits[1]! * 1000 + digits[2]! * 100 + digits[3]! * 10 + digits[4]!;
  return <><div className="grid grid-cols-2 gap-2 sm:grid-cols-5">{["dziesiątki tysięcy", "tysiące", "setki", "dziesiątki", "jedności"].map((label, index) => <PlaceStepper key={label} label={label} value={digits[index]!} disabled={readOnly} onChange={(digit) => { setTouched(true); setSubmitted(false); setDigits((current) => current.map((item, i) => i === index ? digit : item)); }} />)}</div><p className="mt-4 rounded-2xl bg-white/10 p-3 text-center text-4xl font-black">Wynik: {value}</p><button type="button" disabled={readOnly || !touched} onClick={() => setSubmitted(true)} className="mx-auto mt-5 block min-h-12 rounded-xl bg-cyan-200 px-6 font-black text-slate-950 disabled:opacity-40">Zatwierdź</button>{submitted ? <Ready correct={value === expected} answer={String(value)} /> : null}</>;
}
