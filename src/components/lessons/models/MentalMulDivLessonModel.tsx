...
function MentalTask({ taskSeed, readOnly, variant }: { taskSeed: number; readOnly: boolean; variant: number }) {
  ...
  return <Frame title="Mnożenie i dzielenie w pamięci" instruction="Oblicz działanie i zbuduj wynik cyframi wartości pozycyjnych." accent="from-emerald-500 to-teal-900"><p className="mb-5 rounded-3xl bg-white/10 p-5 text-center text-4xl font-black sm:text-6xl">{left} {operator} {right} = □</p><DigitAnswer expected={expected} readOnly={readOnly} /></Frame>;
}
