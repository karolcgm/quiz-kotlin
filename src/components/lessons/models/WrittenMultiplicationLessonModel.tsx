...
export const WRITTEN_MULTIPLICATION_STORY = {
  a: 248,
  b: 36,
  title: "Bilety na festiwal nauki",
  text: "Na festiwal nauki przyjechało 36 autokarów. W każdym autokarze było 248 uczestników. Ilu uczestników przyjechało łącznie?",
  data: ["36 autokarów", "248 uczestników w każdym autokarze"],
  answerPrefix: "Na festiwal przyjechało ",
  answerSuffix: " uczestników.",
} as const;
 ...
 */
export function getWrittenMultiplicationLayout(
  a: number,
  b: number,
...
type ActiveCell = {
  row: "operandA" | "operandB" | "carry" | "partial" | "result";
  rowIndex: number;
  digitIndex: number;
} | null;
...
function WrittenExample({
  ...
  const inputClass = (selected: boolean, small = false) =>
    `grid place-items-center rounded-lg border-2 font-mono font-black transition ${small ? "h-8 w-8 text-base sm:h-9 sm:w-9" : "h-11 w-11 text-xl sm:h-12 sm:w-12 sm:text-2xl"} ${selected ? "border-cyan-500 bg-cyan-100 text-cyan-950 ring-4 ring-cyan-300/50" : "border-slate-300 bg-white text-slate-950"}`;
  const blankClass = "h-11 w-11 sm:h-12 sm:w-12";
  const disabledClass =
    "grid h-11 w-11 place-items-center rounded-lg border-2 border-indigo-700 bg-indigo-600 sm:h-12 sm:w-12";
  ...
  const report = (nextA: string[], nextB: string[], nextResult: string[]) => {
    ...
    onResultChange?.(
      operandsComplete && resultComplete
        ? operandsCorrect && Number(answer) === layout.result
        : null,
      answer || undefined,
    );
  };
  ...
  const change = (digit: string) => {
    ...
    } else if (active.row === "operandA") {
      const next = update(operandAValues);
      setOperandAValues(next);
      report(next, operandBValues, resultValues);
    } else if (active.row === "operandB") {
      const next = update(operandBValues);
      setOperandBValues(next);
      report(operandAValues, next, resultValues);
    } else if (active.row === "partial") {
      setPartialValues((current) =>
        current.map((row, rowIndex) =>
          rowIndex === active.rowIndex ? update(row) : row,
        ),
      );
    } else {
      setCarryValues((current) =>
        current.map((row, rowIndex) =>
          rowIndex === active.rowIndex ? update(row) : row,
        ),
      );
    ...
    if (
      ...
      active.digitIndex <
        (active.row === "operandA"
          ? operandAValues.length
          : operandBValues.length) -
          1
    ) {
      setActive({ ...active, digitIndex: active.digitIndex + 1 });
    } else if (digit !== "←" && active.digitIndex > 0) {
      setActive({ ...active, digitIndex: active.digitIndex - 1 });
    }
  };
    ...
    return Array.from({ length: layout.columns }, (_, column) => {
      ...
      if (carryIndex >= 0 && carryIndex < row.carryCount) {
        return (
          <button
            type="button"
            key={`carry-${rowIndex}-${column}`}
            aria-label={`Przeniesienie, piętro ${rowIndex + 1}, cyfra ${carryIndex + 1}`}
            disabled={readOnly}
            onClick={() =>
              setActive({ row: "carry", rowIndex, digitIndex: carryIndex })
            }
            className={inputClass(
              active?.row === "carry" &&
                active.rowIndex === rowIndex &&
                active.digitIndex === carryIndex,
              true,
            )}
          >
            {carryValues[rowIndex]?.[carryIndex] ?? ""}
          </button>
        );
      }
      return (
        <span
          key={`carry-empty-${rowIndex}-${column}`}
  ...
  const renderNumberCells = (
    value: number,
    rowName: "multiplicand" | "multiplier",
  ) => {
    ...
    return Array.from({ length: layout.columns }, (_, column) =>
      column < start ? (
        ...
        <button
          type="button"
          key={`${rowName}-${column}`}
          aria-label={`${rowName === "multiplicand" ? "Pierwsza liczba" : "Druga liczba"}, cyfra ${column - start + 1}`}
          disabled={readOnly}
          onClick={() =>
            setActive({
              row: rowName === "multiplicand" ? "operandA" : "operandB",
              rowIndex: 0,
              digitIndex: column - start,
            })
          }
          className={inputClass(
            active?.row ===
              (rowName === "multiplicand" ? "operandA" : "operandB") &&
              active.digitIndex === column - start,
          )}
        >
          {
            (rowName === "multiplicand" ? operandAValues : operandBValues)[
              column - start
            ]
          }
        </button>
        ...
        <span
          key={`${rowName}-${column}`}
          className="grid h-11 w-11 place-items-center font-mono text-3xl font-black text-slate-950 sm:h-12 sm:w-12 sm:text-4xl"
        >
          {digits[column - start]}
        </span>
      ),
  ...
  const renderPartialCells = (row: MultiplicationLayoutRow, rowIndex: number) =>
    Array.from({ length: layout.columns }, (_, column) => {
      if (column >= row.digitCount) {
        return (
          <span
            key={`partial-disabled-${rowIndex}-${column}`}
      ...
      return (
        <button
          type="button"
          key={`partial-${rowIndex}-${column}`}
          aria-label={`Iloczyn częściowy, piętro ${rowIndex + 1}, cyfra ${column + 1}`}
          disabled={readOnly}
          onClick={() =>
            setActive({ row: "partial", rowIndex, digitIndex: column })
          }
          className={inputClass(
            active?.row === "partial" &&
              active.rowIndex === rowIndex &&
              active.digitIndex === column,
          )}
        >
          {partialValues[rowIndex]?.[column] ?? ""}
        </button>
      );
    });

  const renderResultCells = () =>
    Array.from({ length: layout.columns }, (_, column) => (
      <button
        type="button"
        key={`result-${column}`}
        aria-label={`Wynik końcowy, cyfra ${column + 1}`}
        disabled={readOnly}
        onClick={() =>
          setActive({ row: "result", rowIndex: 0, digitIndex: column })
        }
        className={inputClass(
          active?.row === "result" && active.digitIndex === column,
        )}
      >
        {resultValues[column]}
      </button>
  ...
  return (
    ...
    >
      <div className="overflow-x-auto pb-2">
        <div
          className="mx-auto grid w-fit items-center gap-2"
          style={{
            gridTemplateColumns: `3rem repeat(${layout.columns}, 3rem)`,
          }}
        >
          {layout.rows[0]?.carryCount
            ? layout.rows.map((row, rowIndex) => (
                <Fragment key={`carry-row-${rowIndex}`}>
                  <span aria-hidden />
                  {renderCarryCells(row, rowIndex)}
                </Fragment>
              ))
            : null}
          <span aria-hidden />
          {renderNumberCells(a, "multiplicand")}
          <span className="text-center text-4xl font-black" aria-label="razy">
            ×
          </span>
          {renderNumberCells(b, "multiplier")}
          <span aria-hidden />
          <span className="col-span-full border-b-4 border-slate-900" />
          {layout.rows.map((row, rowIndex) => (
            <Fragment key={`partial-row-${rowIndex}`}>
              <span className="text-center text-4xl font-black" aria-hidden>
                {rowIndex === layout.rows.length - 1 && layout.rows.length > 1
                  ? "+"
                  : ""}
              </span>
              {renderPartialCells(row, rowIndex)}
            </Fragment>
          ))}
          <span aria-hidden />
          <span className="col-span-full mt-1 border-b-4 border-slate-900" />
          <span className="text-right text-[10px] font-black uppercase text-slate-600">
            wynik
          </span>
          {renderResultCells()}
        </div>
      ...
      {resultComplete ? (
        ...
        >
          {resultCorrect
            ? "Wynik końcowy jest poprawny."
            : "Wynik końcowy jest niepoprawny — popraw najniższy rząd."}
        </p>
      ) : null}
      {story ? (
...
export function WrittenMultiplicationLessonModel({
  seed = 1,
  taskSeed = seed,
  readOnly = false,
  questionNumber,
  ...
  const [localIndex, setLocalIndex] = useState(() =>
    clampTaskIndex((Math.abs(taskSeed) - 1) % FIRST_SLIDE_EXAMPLES.length),
  );
  const storyMode = seed === 2;
  const taskIndex = storyMode
    ? 0
    : questionNumber === undefined
      ? localIndex
      : clampTaskIndex(questionNumber - 1);
  const example = storyMode
    ? WRITTEN_MULTIPLICATION_STORY
    : FIRST_SLIDE_EXAMPLES[taskIndex]!;
  const shownCount = storyMode
    ? 1
    : (questionCount ?? FIRST_SLIDE_EXAMPLES.length);
      ...
      <div className="relative">
        <header className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-black tracking-[.2em] text-cyan-200">
              LICZBY I DZIAŁANIA · TEMAT 7
            </p>
            <h3 className="mt-1 text-3xl font-black sm:text-5xl">
              {storyMode
                ? WRITTEN_MULTIPLICATION_STORY.title
                : "Mnożenie pisemne piętrami"}
            </h3>
            <p className="mt-2 max-w-3xl text-slate-200">
              {storyMode
                ? "Odczytaj dane, wpisz obie liczby do pustych kratek i wykonaj mnożenie pisemne."
                : "Liczba pięter i rzędów przeniesień odpowiada liczbie cyfr mnożnika."}
            </p>
          </div>
        ...
        {storyMode ? (
          <article className="mt-5 rounded-3xl bg-white p-5 text-slate-950 shadow-xl">
            <p className="text-lg font-bold leading-relaxed">
              {WRITTEN_MULTIPLICATION_STORY.text}
            </p>
            <div className="mt-4 rounded-2xl bg-emerald-50 p-4">
              ...
              <ul className="mt-2 space-y-1 font-bold">
                {WRITTEN_MULTIPLICATION_STORY.data.map((item) => (
                  <li key={item}>• {item}</li>
                ))}
              </ul>
            </div>
        ...
        {!storyMode && questionNumber === undefined ? (
          ...
          >
            ...
            <button
              type="button"
              disabled={localIndex === FIRST_SLIDE_EXAMPLES.length - 1}
              onClick={() =>
                setLocalIndex((index) =>
                  Math.min(FIRST_SLIDE_EXAMPLES.length - 1, index + 1),
                )
              }
