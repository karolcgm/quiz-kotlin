...
const shopTasks = [
  ...
  { question: "Czy za cukierek i sok zapłacisz więcej niż 10 zł?", answer: true, products: [{ emoji: "🍬", name: "cukierek", price: "4 zł" }, { emoji: "🧃", name: "sok", price: "8 zł" }] },
];
const storyTasks: EstimateTask[] = [
  ...
  { expression: "Komputer kosztuje 2650 zł, a drukarka 399 zł. Czy za komplet zapłacisz więcej czy mniej niż 3000 zł?", options: ["więcej", "mniej"], answer: "więcej", story: true, title: "Zakupy do pracowni", note: "Porównaj przybliżoną sumę z kwotą 3000 zł." },
  { expression: "Jeden cukierek kosztuje 3 zł. Czy za 10 cukierków zapłacisz więcej czy mniej niż 25 zł?", options: ["więcej", "mniej"], answer: "więcej", story: true, title: "Słodkie zakupy", note: "Oblicz przybliżony koszt zakupów." },
];
...
export function EstimationLessonModel({ seed, taskSeed = seed, readOnly = false, questionNumber, questionCount, onResultChange }: Props) {
  ...
  return <Header title={station === 1 ? "Najpierw oszacuj" : task.title ?? "Zadania z życia"} number={questionNumber} count={questionCount}><p className="mt-5 text-sm font-bold text-amber-100">{task.note ?? "Zaokrąglij liczby do pełnych setek. Nie licz dokładnego wyniku."}</p><p className={`${task.story ? "mt-7 rounded-2xl bg-white/10 p-5 text-xl leading-relaxed" : "mt-7 text-center text-4xl font-black sm:text-6xl"}`}>{task.expression}{task.story ? "" : " ≈ ?"}</p><div className={`mt-7 grid gap-3 ${task.options.length === 2 ? "grid-cols-2" : "sm:grid-cols-3"}`}>{task.options.map((option) => <button key={option} disabled={readOnly} onClick={() => report(option === task.answer, `${option}`)} className="min-h-20 rounded-2xl bg-white text-3xl font-black text-slate-950 shadow-lg disabled:opacity-50">{option}</button>)}</div>{answer ? <p className="mt-4 text-center font-bold text-amber-100">Odpowiedź wybrana — wyślij ją.</p> : null}</Header>;
}
