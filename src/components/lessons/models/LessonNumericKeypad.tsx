...
export function LessonNumericKeypad({ onKey, onConfirm, disabled = false, allowSeparator = false, allowNegative = false, operationKeys = [], selectedOperation, label = "Klawiatura ekranowa", helperText }: Props) {
  ...
  return <section className="rounded-2xl bg-slate-900 p-3 text-white shadow-lg" aria-label={label} data-lesson-numeric-keypad="shared">
    ...
    <div className="mx-auto grid max-w-md grid-cols-4 gap-2">
      ...
      {allowNegative ? <button type="button" disabled={disabled} onPointerDown={(event) => runOnPointerDown(event, () => onKey("minus"))} onPointerCancel={() => { pointerHandledRef.current = false; }} onClick={() => runOnClick(() => onKey("minus"))} className="min-h-12 rounded-xl bg-amber-200 text-lg font-black text-amber-950 disabled:opacity-35">− minus</button> : <span aria-hidden />}
      {operationKeys.map((key) => <button key={`operation-${key}`} type="button" disabled={disabled} aria-pressed={selectedOperation === key} onPointerDown={(event) => runOnPointerDown(event, () => onKey(key))} onPointerCancel={() => { pointerHandledRef.current = false; }} onClick={() => runOnClick(() => onKey(key))} className={`min-h-12 rounded-xl text-2xl font-black shadow focus-visible:outline focus-visible:outline-4 focus-visible:outline-cyan-300 disabled:opacity-35 ${selectedOperation === key ? "bg-violet-600 text-white ring-4 ring-violet-300" : "bg-cyan-200 text-cyan-950"}`}>{key}</button>)}
