...
export function WrittenAddSubLessonModel({ seed, taskSeed = seed, readOnly = false, questionNumber, questionCount, onResultChange }: Props) {
  ...
  const expected = subtract ? a - b : a + b; const columns = Math.max(String(a).length, String(b).length, String(expected).length);
  ...
  const change = (digit:string) => {
    ...
    set(current => { const next=[...current]; next[active.column]=digit === "←" ? "" : digit; if(active.row === "result") { const value=next.join(""); onResultChange?.(value ? Number(value) === expected : null, value || undefined); } return next; });
  ...
  return <section className="relative overflow-hidden rounded-[2rem] bg-slate-950 p-5 text-white shadow-2xl sm:p-8"><div className="absolute inset-0 bg-gradient-to-br from-cyan-500/30 via-indigo-700/20 to-violet-700/35"/><div className="relative"><header className="flex items-start justify-between gap-3"><div><p className="text-xs font-black tracking-[.2em] text-cyan-200">LICZBY I DZIAŁANIA · TEMAT 6</p><h3 className="mt-1 text-3xl font-black sm:text-5xl">{subtract ? "Odejmowanie pisemne" : "Dodawanie pisemne"}</h3><p className="mt-2 text-cyan-50">Uzupełnij przeniesienia w małych kratkach, a wynik wpisz cyfra po cyfrze w dolnych kratkach.</p></div>{questionNumber && questionCount ? <b className="rounded-2xl bg-cyan-300 px-4 py-2 text-sm text-slate-950">Zadanie {questionNumber}/{questionCount}</b> : null}</header>
    <div className="mx-auto mt-7 w-fit rounded-3xl bg-slate-100 p-4 shadow-xl sm:p-6"><div className="mb-2 flex items-center justify-end gap-2"><span className="mr-2 text-xs font-black uppercase tracking-wide text-slate-500">Przeniesienia</span>{Array.from({length:columns},(_,column)=><button type="button" key={`carry-${column}`} disabled={readOnly} onClick={()=>setActive({row:"carry",column})} className={cellClass("carry",column,true)}>{carries[column]}</button>)}</div><div className="grid items-center gap-2" style={{gridTemplateColumns:`2rem repeat(${columns}, minmax(0, 1fr))`}}><span/>{Array.from({length:columns},(_,column)=><span key={`a-${column}`} className="grid h-14 w-14 place-items-center font-mono text-3xl font-black text-slate-950 sm:h-16 sm:w-16 sm:text-4xl">{digitAt(a,column,columns)}</span>)}<span className="text-3xl font-black text-slate-950">{subtract?"−":"+"}</span>{Array.from({length:columns},(_,column)=><span key={`b-${column}`} className="grid h-14 w-14 place-items-center font-mono text-3xl font-black text-slate-950 sm:h-16 sm:w-16 sm:text-4xl">{digitAt(b,column,columns)}</span>)}<span className="col-span-full mt-1 border-b-4 border-slate-900"/><span/>{Array.from({length:columns},(_,column)=><button type="button" key={`result-${column}`} disabled={readOnly} onClick={()=>setActive({row:"result",column})} className={cellClass("result",column)}>{resultDigits[column]}</button>)}</div></div>
