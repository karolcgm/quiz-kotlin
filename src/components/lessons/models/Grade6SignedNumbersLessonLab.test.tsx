...
describe("Grade6SignedNumbersLessonLab V2", () => {
  ...
  it("zachowuje rozrzucone litery po odmontowaniu zadania i pokazuje pełne hasło dopiero na końcu", () => {
    const onResultChange = vi.fn();
    ...
    fireEvent.click(screen.getByRole("button", { name: "Zatwierdź" }));
    expect(screen.getByRole("region", { name: "Odszyfrowane hasło" }).textContent).toContain("??T???????");
    expect(onResultChange).toHaveBeenLastCalledWith(true, "-8");
    first.unmount();
    ...
    expect(screen.getByRole("region", { name: "Odszyfrowane hasło" }).textContent).toContain("M?T??A?Y??");
    fifth.unmount();
    ...
    expect(screen.getByRole("region", { name: "Odszyfrowane hasło" }).textContent).toContain("MATE?ATYKA");
    fireEvent.click(screen.getByRole("button", { name: "1" }));
    fireEvent.click(screen.getByRole("button", { name: "Zatwierdź" }));
