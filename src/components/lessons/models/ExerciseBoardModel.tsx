...
export function ExerciseBoardModel({ readOnly = false }: Props) {
  ...
  return <section className="relative isolate overflow-hidden rounded-[2rem] bg-slate-950 p-5 text-white shadow-2xl sm:p-8">
    ...
    <div className="mt-8 grid grid-cols-3 gap-3 sm:grid-cols-4 lg:grid-cols-6">{Array.from({ length: 18 }).map((_, index) => { const active = marked.includes(index); return <button key={index} type="button" onClick={() => toggle(index)} className={`aspect-[1.2] rounded-2xl border-2 text-3xl font-black transition active:scale-95 sm:text-4xl ${active ? "border-emerald-200 bg-emerald-400 text-emerald-950 shadow-[0_0_30px_rgba(52,211,153,.45)]" : "border-white/15 bg-white/5 text-white hover:border-cyan-200 hover:bg-white/10"}`} aria-pressed={active}><span className="block">{index + 1}</span><span className="mt-1 block text-[10px] tracking-widest">{active ? "WYBRANE" : "DOTKNIJ"}</span></button>; })}</div>
