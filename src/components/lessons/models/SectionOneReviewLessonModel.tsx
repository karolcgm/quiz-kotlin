...
export const NUMBER_DECODER_TASKS: readonly ChoiceTask[] = [
  {
    ...
    correct:
      "12 542 385 412 — dwanaście miliardów pięćset czterdzieści dwa miliony trzysta osiemdziesiąt pięć tysięcy czterysta dwanaście",
    explanation:
      "Cyfry dzielimy od prawej strony na grupy po trzy: jedności, tysiące, miliony i miliardy.",
  },
  {
    ...
    correct:
      "trzy miliony dwieście pięćdziesiąt osiem tysięcy czterysta dwanaście",
    explanation:
      "Pierwsza grupa oznacza miliony, druga tysiące, a ostatnia jedności.",
  },
  {
    ...
    explanation:
      "Brak milionów i jedności zapisujemy zerami: 3 | 000 | 500 | 000.",
  },
  {
...
export const REVIEW_MENTAL_TASKS = [
  {
    expression: "850 + 330",
    answer: 1180,
    hint: "Dodaj najpierw setki, potem dziesiątki.",
  },
  {
    expression: "4050 − 100",
    answer: 3950,
    hint: "Odejmujesz dokładnie jedną setkę.",
  },
  {
    expression: "40 × 500",
    answer: 20_000,
    hint: "Najpierw 4 × 5, potem dopisz trzy zera.",
  },
  {
    expression: "770 000 ÷ 100",
    answer: 7700,
    hint: "Dzielenie przez 100 usuwa dwa zera.",
  },
] as const;
...
export const REVIEW_ORDER_TASKS: readonly ChoiceTask[] = [
  ...
  },
  {
    ...
    correct: "32",
    explanation:
      "Dzielenie i mnożenie wykonujemy od lewej: 24 ÷ 6 = 4, 4 × 4 = 16, 48 − 16 = 32.",
  },
  {
...
function ChoiceMission({
  ...
  const choices = useMemo(
    () => seededShuffle(task.choices, taskSeed),
    [task.choices, taskSeed],
  );
  ...
  return (
    <article
      className={`rounded-[2rem] p-5 shadow-2xl sm:p-8 ${decoder ? "bg-gradient-to-br from-slate-950 via-indigo-950 to-cyan-950 text-white" : "bg-gradient-to-br from-amber-100 via-orange-50 to-fuchsia-100 text-slate-950"}`}
    >
      <p
        className={`text-xs font-black uppercase tracking-[.18em] ${decoder ? "text-cyan-200" : "text-fuchsia-700"}`}
      >
        {task.eyebrow}
      </p>
      <h4 className="mt-2 text-2xl font-black sm:text-3xl">{task.prompt}</h4>
      <div
        className={`mt-5 rounded-3xl border-4 p-5 text-center font-mono font-black tabular-nums shadow-inner [font-size:clamp(1.5rem,5vw,3.4rem)] ${decoder ? "border-cyan-400/40 bg-black/30 text-cyan-200" : "border-amber-300 bg-slate-950 text-amber-200"}`}
      >
        {task.display}
      </div>
...
function NumberLineMission({
  task,
  taskSeed,
  readOnly,
  onResultChange,
}: {
  task: ReviewNumberLineTask;
  taskSeed: number;
  readOnly: boolean;
  onResultChange: Props["onResultChange"];
}) {
  ...
  return (
    <article className="rounded-[2rem] bg-gradient-to-br from-sky-100 via-white to-violet-100 p-5 text-slate-950 shadow-2xl sm:p-8">
      ...
      <h4 className="mt-2 text-3xl font-black">
        Na jakiej liczbie zatrzymał się wagon?
      </h4>
      <p className="mt-2 font-bold text-slate-600">
        Odstęp między sąsiednimi kreskami jest zawsze taki sam.
      </p>
      <div className="mt-8 overflow-x-auto rounded-3xl border-4 border-sky-200 bg-sky-950 px-5 pb-7 pt-10 text-white shadow-inner">
        <div className="relative grid min-w-[44rem] grid-cols-11">
          <div
            className="absolute left-0 right-0 top-7 h-2 rounded-full bg-gradient-to-r from-cyan-300 via-white to-violet-300"
            aria-hidden
          />
          {Array.from({ length: 11 }, (_, index) => {
            ...
            return (
              <div
                key={index}
                className="relative z-10 flex min-h-24 flex-col items-center"
              >
                {marker ? (
                  <span
                    className="absolute -top-9 text-3xl"
                    aria-label="Szukany przystanek"
                  >
                    🚀
                  </span>
                ) : null}
                <span
                  className={`mt-4 h-7 w-1.5 rounded-full ${marker ? "bg-amber-300" : "bg-white"}`}
                  aria-hidden
                />
                <span
                  className={`mt-2 min-h-8 rounded-lg px-2 py-1 text-sm font-black ${marker ? "bg-amber-300 text-slate-950" : known ? "bg-white/15" : "text-white/40"}`}
                >
                  {marker ? "?" : (known?.value ?? "")}
                </span>
              </div>
      ...
      <MissionStatus
        correct={correct}
        success={`Tak — każdy krok ma wartość ${task.step}, więc rakieta stoi na liczbie ${answer}.`}
        error="Sprawdź różnicę między dwiema podpisanymi kreskami i policz równe kroki."
      />
    </article>
  );
}
...
function SortMission({
  ...
  const candidates = useMemo(
    () => seededShuffle(numbers, taskSeed),
    [numbers, taskSeed],
  );
  ...
  const check = () => {
    const correct =
      ...
      expected.every((value, index) => selected[index] === value);
    setChecked(correct);
    onResultChange?.(
      correct,
      selected.map((value) => value.toLocaleString("pl-PL")).join(" < "),
    );
  };
  ...
  return (
    <article className="rounded-[2rem] bg-gradient-to-br from-emerald-950 via-teal-950 to-slate-950 p-5 text-white shadow-2xl sm:p-8">
      <p className="text-xs font-black uppercase tracking-[.18em] text-emerald-200">
        Sortownia ładunków
      </p>
      <h4 className="mt-2 text-3xl font-black">
        Ustaw kontenery od najmniejszej liczby do największej
      </h4>
      <p className="mt-2 text-emerald-100">
        Klikaj liczby w takiej kolejności, w jakiej mają wjechać na taśmę.
      </p>
      <div className="mt-6 grid min-h-24 grid-cols-2 gap-2 rounded-3xl border-2 border-dashed border-emerald-300/50 bg-black/20 p-3 sm:grid-cols-5">
        {Array.from({ length: numbers.length }, (_, index) => (
          <div
            key={index}
            className="grid min-h-16 place-items-center rounded-2xl bg-white/10 px-2 text-center font-black"
          >
            {selected[index]?.toLocaleString("pl-PL") ?? (
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        <button
          ...
          onClick={() => change(selected.slice(0, -1))}
          className="min-h-12 rounded-2xl border border-white/30 font-black disabled:opacity-30"
        >
          Cofnij ostatni kontener
        </button>
        <button
          type="button"
          disabled={readOnly || selected.length !== numbers.length}
          onClick={check}
          className="min-h-12 rounded-2xl bg-emerald-300 font-black text-emerald-950 disabled:opacity-30"
        >
          Sprawdź kolejność
        </button>
      </div>
      <MissionStatus
        correct={checked}
        success="Taśma działa — wszystkie kontenery stoją rosnąco."
        error="Co najmniej dwa kontenery są zamienione miejscami. Porównuj liczby od najwyższego rzędu."
      />
    </article>
  );
}

function NumericMission({
  task,
  readOnly,
  onResultChange,
}: {
  task: (typeof REVIEW_MENTAL_TASKS)[number];
  readOnly: boolean;
  onResultChange: Props["onResultChange"];
}) {
  ...
  return (
    <article className="rounded-[2rem] bg-gradient-to-br from-fuchsia-950 via-violet-950 to-indigo-950 p-5 text-white shadow-2xl sm:p-8">
      <p className="text-xs font-black uppercase tracking-[.18em] text-fuchsia-200">
        Reaktor pamięciowy
      </p>
      <h4 className="mt-2 text-3xl font-black">
        Naładuj rdzeń bez pisemnego algorytmu
      </h4>
      <div className="mx-auto mt-7 max-w-2xl rounded-[2rem] border-4 border-fuchsia-300/40 bg-black/30 p-6 text-center shadow-inner">
        <p className="font-mono font-black tabular-nums text-fuchsia-100 [font-size:clamp(2.4rem,8vw,5rem)]">
          {task.expression}
        </p>
        <div className="mx-auto mt-5 h-4 max-w-md overflow-hidden rounded-full bg-white/10">
          <div
            className="h-full rounded-full bg-gradient-to-r from-cyan-300 to-fuchsia-300 transition-all"
            style={{
              width: answer
                ? `${Math.min(100, 25 + answer.length * 18)}%`
                : "8%",
            }}
          />
        </div>
      ...
      <label className="mx-auto mt-6 block max-w-md text-center text-lg font-black">
        Wynik
        <input
          aria-label="Wynik działania pamięciowego"
          inputMode="none"
          disabled={readOnly}
          value={answer}
          onChange={(event) => update(event.target.value)}
          className="mt-2 min-h-14 w-full rounded-2xl border-2 border-fuchsia-300 bg-white px-3 text-center text-3xl font-black text-slate-950"
        />
      </label>
      <div className="mx-auto mt-4 max-w-xl rounded-3xl bg-white p-3 text-slate-950">
        <NumericLessonKeypad
          onKey={(key) =>
            update(
              key === "backspace" ? answer.slice(0, -1) : `${answer}${key}`,
            )
          }
          disabled={readOnly}
          label="Klawiatura reaktora"
        />
      </div>
      <button
        type="button"
        ...
        onClick={check}
        className="mt-5 min-h-14 w-full rounded-2xl bg-fuchsia-300 text-lg font-black text-fuchsia-950 disabled:opacity-30"
      >
        Uruchom reaktor
      </button>
      <MissionStatus
        correct={checked}
        success={`Rdzeń pracuje! ${task.expression} = ${task.answer.toLocaleString("pl-PL")}.`}
        error={task.hint}
      />
    </article>
  );
...
function RemainderMission({
  task,
  readOnly,
  onResultChange,
}: {
  task: (typeof REVIEW_REMAINDER_TASKS)[number];
  readOnly: boolean;
  onResultChange: Props["onResultChange"];
}) {
  ...
  const check = () => {
    const correct =
      Number(quotient) === task.quotient &&
      Number(remainder) === task.remainder;
    setChecked(correct);
  ...
  return (
    <article className="rounded-[2rem] bg-gradient-to-br from-amber-100 via-orange-50 to-cyan-100 p-5 text-slate-950 shadow-2xl sm:p-8">
      ...
      <h4 className="mt-2 text-3xl font-black">
        Zapakuj {task.total} sztuk: po {task.divisor} w każdym pojemniku
      </h4>
      <p className="mt-2 text-lg font-bold text-slate-700">
        Ładunek: {task.cargo}. Ile pełnych pojemników powstanie i ile sztuk
        zostanie?
      </p>
      <div className="mt-6 grid items-center gap-4 rounded-3xl border-4 border-orange-200 bg-white/80 p-5 sm:grid-cols-[1fr_auto_1fr_auto_1fr]">
        ...
        <span
          className="text-center text-3xl font-black text-orange-600"
          aria-hidden
        >
          →
        </span>
      ...
      <div className="mx-auto mt-6 grid max-w-2xl gap-4 sm:grid-cols-2">
        <label className="text-center text-lg font-black">
          Pełne pojemniki
          <input
            aria-label="Liczba pełnych pojemników"
            inputMode="none"
            disabled={readOnly}
            value={quotient}
            onFocus={() => setActive("quotient")}
            onClick={() => setActive("quotient")}
            onChange={(event) => update("quotient", event.target.value)}
            className={`mt-2 min-h-14 w-full rounded-2xl border-4 bg-white text-center text-3xl font-black ${active === "quotient" ? "border-cyan-500" : "border-slate-200"}`}
          />
        </label>
        <label className="text-center text-lg font-black">
          Reszta
          <input
            aria-label="Reszta z dzielenia"
            inputMode="none"
            disabled={readOnly}
            value={remainder}
            onFocus={() => setActive("remainder")}
            onClick={() => setActive("remainder")}
            onChange={(event) => update("remainder", event.target.value)}
            className={`mt-2 min-h-14 w-full rounded-2xl border-4 bg-white text-center text-3xl font-black ${active === "remainder" ? "border-orange-500" : "border-slate-200"}`}
          />
        </label>
      </div>
      <div className="mx-auto mt-4 max-w-xl rounded-3xl bg-white p-3">
        <NumericLessonKeypad
          onKey={applyKey}
          disabled={readOnly}
          label="Klawiatura pakowalni"
        />
      </div>
      <button
        type="button"
        disabled={readOnly || !quotient || !remainder}
        onClick={check}
        className="mt-5 min-h-14 w-full rounded-2xl bg-slate-950 text-lg font-black text-white disabled:opacity-30"
      >
        Sprawdź pakowanie
      </button>
      <MissionStatus
        correct={checked}
        success={`${task.total} = ${task.divisor} × ${task.quotient} + ${task.remainder}. Reszta jest mniejsza od dzielnika.`}
        error="Sprawdź: dzielnik × liczba pełnych pojemników + reszta musi dać cały ładunek, a reszta musi być mniejsza od dzielnika."
      />
    </article>
  );
}

function WrittenReviewMission({
  taskIndex,
  taskSeed,
  readOnly,
  onResultChange,
...
const STATION_TITLES = [
  ...
  "Hala działań pisemnych",
] as const;

export function SectionOneReviewLessonModel({
  seed = 1,
  taskSeed = 1,
  readOnly = false,
  questionNumber = 1,
  ...
  const station = Math.min(7, Math.max(1, seed));
  const taskIndex = Math.max(0, questionNumber - 1) % 4;
  const stableTaskSeed = taskSeed + station * 1009 + taskIndex * 97;

  return (
    <section
      data-section-one-review-station={station}
      className="rounded-[2.25rem] bg-gradient-to-br from-indigo-700 via-violet-700 to-cyan-600 p-3 shadow-2xl sm:p-5"
    >
      <header className="mb-4 flex flex-wrap items-start justify-between gap-3 px-2 text-white">
        <div>
          <p className="text-xs font-black uppercase tracking-[.2em] text-cyan-100">
            Powtórzenie Działu I · Misja {station}/7
          </p>
          <h3 className="mt-1 text-2xl font-black sm:text-4xl">
            {STATION_TITLES[station - 1]}
          </h3>
        </div>
        <b className="rounded-2xl bg-white/20 px-4 py-2">
          Mini-stacja {questionNumber}/{questionCount}
        </b>
      </header>
      {station === 1 ? (
        <ChoiceMission
          key={taskIndex}
          task={NUMBER_DECODER_TASKS[taskIndex]!}
          taskSeed={stableTaskSeed}
          readOnly={readOnly}
          onResultChange={onResultChange}
          theme="decoder"
        />
      ...
      {station === 2 ? (
        <NumberLineMission
          key={taskIndex}
          task={REVIEW_NUMBER_LINE_TASKS[taskIndex]!}
          taskSeed={stableTaskSeed}
          readOnly={readOnly}
          onResultChange={onResultChange}
        />
      ...
      {station === 3 ? (
        <SortMission
          key={taskIndex}
          numbers={REVIEW_SORT_TASKS[taskIndex]!}
          taskSeed={stableTaskSeed}
          readOnly={readOnly}
          onResultChange={onResultChange}
        />
      ...
      {station === 4 ? (
        <NumericMission
          key={taskIndex}
          task={REVIEW_MENTAL_TASKS[taskIndex]!}
          readOnly={readOnly}
          onResultChange={onResultChange}
        />
      ) : null}
      {station === 5 ? (
        <ChoiceMission
          key={taskIndex}
          task={REVIEW_ORDER_TASKS[taskIndex]!}
          taskSeed={stableTaskSeed}
          readOnly={readOnly}
          onResultChange={onResultChange}
          theme="circuit"
      ...
      {station === 6 ? (
        <RemainderMission
          key={taskIndex}
          task={REVIEW_REMAINDER_TASKS[taskIndex]!}
          readOnly={readOnly}
          onResultChange={onResultChange}
        />
      ) : null}
      {station === 7 ? (
        <WrittenReviewMission
          key={taskIndex}
          taskIndex={taskIndex}
          taskSeed={stableTaskSeed}
          readOnly={readOnly}
