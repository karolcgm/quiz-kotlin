...
function MachineScene({ progress, paused }: { progress: number; paused: boolean }) {
  ...
  const targetX = machineTokenTargetX(progress);
  useFrame(({ clock }) => {
    if (!token.current) return;
    if (!paused) token.current.position.x += (targetX - token.current.position.x) * 0.08;
    token.current.position.y = paused ? 0 : Math.sin(clock.elapsedTime * 3) * 0.05;
  ...
  return <>
    ...
    <group position={[0, 0, 0]}>
      ...
      <mesh ref={token} castShadow position={[-4.8, 0, 1.25]}>
...
export function machineTokenTargetX(progress: number) {
  return [-4.8, -2.3, 0.8, 3.9][Math.max(0, Math.min(3, progress))] ?? -4.8;
}

export function AlgebraMachineScene3D({ input = 4, inputLabel, progress = 0, labels = ["Podstaw x", "Wykonaj mnożenie", "Dodaj lub odejmij", "Odczytaj wynik"], stepValues }: { input?: number; inputLabel?: string; progress?: number; labels?: string[]; stepValues?: ReactNode[] }) {
  ...
  const machineOverlay = <div className="pointer-events-none absolute inset-x-2 bottom-3 grid grid-cols-[minmax(0,.9fr)_minmax(0,1fr)_minmax(0,1.35fr)_minmax(0,.75fr)] gap-1 sm:inset-x-4 sm:gap-2" data-machine-values>
    {visibleValues.map((value, index) => <div key={index} data-machine-output={index === visibleValues.length - 1 ? "true" : undefined} data-machine-value={index} className={`min-w-0 overflow-hidden rounded-xl border-2 px-1 py-2 text-center font-mono text-xs font-black shadow-lg backdrop-blur-sm sm:px-2 sm:text-sm ${index === safeProgress ? "border-amber-300 bg-amber-300 text-amber-950 ring-4 ring-amber-100/70" : index < safeProgress ? "border-emerald-300 bg-emerald-100/95 text-emerald-950" : "border-white/50 bg-slate-950/85 text-white"}`}><span className="block text-[.55rem] font-black uppercase tracking-wider opacity-75 sm:text-[.62rem]">{index === 0 ? "wejście" : index === visibleValues.length - 1 ? "wyjście" : `komora ${index}`}</span><span className="mt-1 flex min-h-5 min-w-0 items-center justify-center" data-machine-value-content>{value}</span></div>)}
  </div>;
