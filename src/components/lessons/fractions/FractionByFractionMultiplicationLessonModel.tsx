...
function MultiplicationRound({ task, locked, directCalculation = false, onComplete, onIncorrect }: { task: MultiplicationTask; locked: boolean; directCalculation?: boolean; onComplete: (answer: string) => void; onIncorrect: () => void }) {
  const structure = useMemo(() => buildFields(task), [task]);
  const { fields, setupIds, workingLeftId, workingRightId } = structure;
  const firstCalculationFieldIndex = Math.max(0, fields.findIndex((field) => !setupIds.has(field.id)));
  const initialField = fields[directCalculation ? firstCalculationFieldIndex : 0]!;
  const [entries, setEntries] = useState<Record<string, FieldEntry>>(() => directCalculation ? prefilledSetupEntries(fields, setupIds) : blankEntries(fields));
  ...
  const [activePart, setActivePart] = useState<FieldPart>(initialField.kind === "mixed" ? "wholePart" : initialField.kind === "fraction" ? "numerator" : "integer");
  ...
  const givenOperation = <div className={directCalculation ? "sr-only" : "grid justify-items-center gap-2 rounded-2xl border-2 border-indigo-200 bg-indigo-50 p-3"} aria-label="Działanie do rozwiązania"><span className={directCalculation ? "sr-only" : "text-sm font-black text-indigo-900"}>Działanie do rozwiązania</span><div className="flex flex-wrap items-center justify-center gap-3 text-2xl font-black"><span data-given-multiplication-left>{isMixed(task.left) ? <StaticMixed value={task.left} /> : <StaticFraction value={improper(task.left)} />}</span><b>·</b><span data-given-multiplication-right>{isMixed(task.right) ? <StaticMixed value={task.right} /> : <StaticFraction value={improper(task.right)} />}</span></div></div>;
  return <div className="grid gap-4">{task.story ? <section className="rounded-2xl border-2 border-emerald-300 bg-emerald-50 p-4"><p className="text-xs font-black uppercase tracking-wide text-emerald-800">Zadanie tekstowe</p><p className="mt-2 text-lg font-bold leading-relaxed">{task.story}</p></section> : null}<section className="grid gap-3 rounded-2xl border-2 border-slate-200 bg-white p-4">{task.story ? null : <>{!directCalculation ? <p className="text-xs font-black uppercase tracking-wide text-indigo-700">Twoje zadanie</p> : null}{givenOperation}</>}<h3 className="font-black">{task.prompt}</h3><div className="flex max-w-full flex-wrap items-center justify-center gap-3 overflow-x-auto px-3 py-7 text-xl font-black" aria-label="Pełny zapis mnożenia ułamków">{line}{setupComplete ? <><b>=</b>{renderField("result")}{fields.some((field) => field.id === "mixed-result") ? <><b>=</b>{renderField("mixed-result")}</> : null}{task.unit ? <b>{task.unit}</b> : null}</> : null}</div>{answerLine}<p className={`text-center text-sm font-bold ${setupComplete ? "text-emerald-800" : "text-indigo-800"}`}>{setupComplete ? Object.keys(leftReplacements).length + Object.keys(rightReplacements).length > 0 ? "Wpisz w małych kratkach liczby po skróceniu, a następnie oblicz wynik." : "Pomnóż liczniki i mianowniki, a następnie wpisz wynik." : task.story ? "Etap 1: zapisz ułamek z ułamka i zamień ten zapis na mnożenie." : isMixed(task.left) && isMixed(task.right) ? "Etap 1: przepisz obie liczby mieszane i zamień każdą na ułamek niewłaściwy." : isMixed(task.left) ? "Etap 1: przepisz podane działanie, a potem zamień liczbę mieszaną na ułamek niewłaściwy." : "Etap 1: przepisz do kratek oba ułamki z działania powyżej."}</p></section>{!locked ? <LessonNumericKeypad label="Kalkulator do mnożenia ułamków" helperText={setupComplete ? task.story ? "Uzupełnij wartości po skróceniu, wynik i odpowiedź." : "Uzupełnij wartości po skróceniu i wynik." : task.story ? "Najpierw uzupełnij cały pierwszy etap." : "Przepisz do kratek działanie pokazane nad nimi."} onKey={edit} onConfirm={confirm} /> : null}{feedback ? <p role="status" className="rounded-xl border-2 border-rose-300 bg-rose-50 p-3 font-black text-rose-900">{feedback}</p> : null}</div>;
}
