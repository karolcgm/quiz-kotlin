...
function MultiplicationRound({ task, locked, onComplete, onIncorrect }: { task: MultiplicationTask; locked: boolean; onComplete: (answer: string) => void; onIncorrect: () => void }) {
  ...
  return <div className="grid gap-4">{task.story ? <section className="rounded-2xl border-2 border-emerald-300 bg-emerald-50 p-4"><p className="text-xs font-black uppercase tracking-wide text-emerald-800">Zadanie tekstowe</p><p className="mt-2 text-lg font-bold leading-relaxed">{task.story}</p></section> : null}<section className="grid gap-3 rounded-2xl border-2 border-slate-200 bg-white p-4"><h3 className="font-black">{task.prompt}</h3><div className="flex max-w-full flex-wrap items-center justify-center gap-3 overflow-x-auto px-3 py-7 text-xl font-black" aria-label="Pełny zapis mnożenia ułamków">{line}{setupComplete ? <><b>=</b>{renderField("result")}{fields.some((field) => field.id === "mixed-result") ? <><b>=</b>{renderField("mixed-result")}</> : null}{task.unit ? <b>{task.unit}</b> : null}</> : null}</div><p className={`text-center text-sm font-bold ${setupComplete ? "text-emerald-800" : "text-indigo-800"}`}>{setupComplete ? Object.keys(leftReplacements).length + Object.keys(rightReplacements).length > 0 ? "Zapis jest zablokowany. Wpisz w małych kratkach liczby po skróceniu, a następnie oblicz wynik." : "Zapis jest zablokowany. Pomnóż liczniki i mianowniki, a następnie wpisz wynik." : task.story ? "Etap 1: zapisz ułamek z ułamka i zamień ten zapis na mnożenie." : task.left.wholePart > 0 ? "Etap 1: zamień liczbę mieszaną na ułamek niewłaściwy." : "Etap 1: przepisz oba ułamki działania."}</p></section>{!locked ? <LessonNumericKeypad label="Kalkulator do mnożenia ułamków" helperText={setupComplete ? "Uzupełnij wartości po skróceniu i wynik." : "Najpierw uzupełnij cały pierwszy etap."} onKey={edit} onConfirm={confirm} /> : null}{feedback ? <p role="status" className="rounded-xl border-2 border-rose-300 bg-rose-50 p-3 font-black text-rose-900">{feedback}</p> : null}</div>;
}
