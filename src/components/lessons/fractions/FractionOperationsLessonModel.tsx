...
function FractionOperationsLessonModelContent({ activity, seed, taskSeed, readOnly = false, presentationMode = false, questionNumber, questionCount, onResultChange }: Props) {
  ...
  return (
    <article className="relative isolate overflow-hidden rounded-[2rem] bg-slate-950 p-4 text-white shadow-2xl sm:p-7" data-fraction-operations data-topic={topic} data-level={level} data-phase={phase}>
      <div className="absolute inset-0 -z-10 bg-gradient-to-br from-indigo-700 via-slate-950 to-cyan-900 opacity-60" />
      <header className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.2em] text-cyan-200">Ułamki zwykłe · Temat {topic} · {level}</p><h2 className="mt-1 text-2xl font-black sm:text-4xl">{TOPIC_TITLES[topic]}</h2><p className="mt-2 max-w-3xl text-sm font-semibold text-slate-200 sm:text-lg">{phase === "independent" ? task.prompt : "Klikaj elementy modelu, obserwuj zmianę zapisu i nazywaj każdy krok."}</p></div>{questionNumber && questionCount ? <b className="shrink-0 rounded-2xl bg-cyan-300 px-4 py-2 text-sm font-black text-slate-950">Zadanie {questionNumber}/{questionCount}</b> : null}</header>
