...
const ACTIVITY_TITLES: Record<FractionLessonL2Activity, string> = {
  ...
  "independent-l2": "Zamiana ułamków i liczb mieszanych",
};
...
export function FractionLessonL2Model({
  ...
  const convertedFractionCells = {
    numerator: String(task.target.numerator).length,
    denominator: String(task.target.denominator).length,
  };
    ...
    <LessonTaskFrame className={styles.lesson} contentClassName="space-y-4" eyebrow="Dział 3 · Ułamki zwykłe" heading={ACTIVITY_TITLES[activity]} description={task.prompt} questionNumber={questionNumber} questionCount={questionCount} data-fraction-lesson-l2 data-fraction-activity={activity} data-orientation-contract="portrait-landscape" data-generator-id={task.generatorId} data-seed={effectiveSeed} data-difficulty={difficulty}>
      ...
      {activity === "convert-both-ways" || activity === "extract-wholes" || activity === "mixed-to-improper" ? <div className="space-y-4">
        {activity === "convert-both-ways" ?
          ...
          : activity === "extract-wholes" ? <div className="rounded-2xl border-2 border-violet-200 bg-violet-50 p-3 text-center font-bold">Podziel licznik przez mianownik. Iloraz zapiszesz jako całość, a resztę jako licznik części ułamkowej.</div> : null}
        {activity === "mixed-to-improper" ? <div className="flex flex-wrap items-center justify-center gap-4 py-5 text-2xl font-black"><StaticMixed value={task.mixed} label="Liczba mieszana" /><span aria-hidden>=</span><FractionStackInput value={response} onChange={(value) => { setResponse(value); clearResult(); }} fixedDigitCells={convertedFractionCells} readOnly={controlsLocked} stepLabel="Wpisz ułamek niewłaściwy" /></div> : <><div className={styles.conversionGrid}><div className={styles.sourceCard}>{direction === "improper-to-mixed" ? <StaticFraction value={task.target} label="Ułamek niewłaściwy" /> : <StaticMixed value={task.mixed} label="Liczba mieszana" />}</div><div className={styles.connector} data-conversion-connector>{activity === "extract-wholes" ? <><span>dzielimy licznik przez mianownik</span><b>{revealStep >= 1 ? `${task.target.numerator} : ${task.target.denominator}` : "□ : □"}</b><b>{revealStep >= 2 ? `= ${task.mixed.wholePart}` : "= □"}</b><b>{revealStep >= 3 ? `reszta ${task.mixed.numerator}` : "reszta □"}</b></> : <><span>całości × mianownik + licznik</span><b>{revealStep >= 1 ? `${task.mixed.wholePart} × ${task.mixed.denominator}` : "□ × □"}</b><b>{revealStep >= 2 ? `+ ${task.mixed.numerator}` : "+ □"}</b><b>{revealStep >= 3 ? `= ${equation.result}` : "= □"}</b></>}</div><div className={styles.responseCard}><FractionStackInput value={response} onChange={(value) => { setResponse(value); clearResult(); }} showWholePart={direction === "improper-to-mixed"} readOnly={controlsLocked} stepLabel="Wpisz liczbę mieszaną" /></div></div><div className="flex flex-wrap justify-center gap-2"><button type="button" className={styles.secondaryButton} disabled={revealStep === 0} onClick={() => setRevealStep((step) => Math.max(0, step - 1))}>← Poprzedni krok</button><button type="button" className={styles.secondaryButton} disabled={revealStep === 3} onClick={() => setRevealStep((step) => Math.min(3, step + 1))}>Następny krok →</button></div></>}
      ...
      {activity === "independent-l2" ? <div className="space-y-4"><div className="flex flex-wrap items-center justify-center gap-3 rounded-2xl border-2 border-violet-200 bg-white p-4"><span className="font-black">Wylosowany zapis:</span>{sourceDisplay}</div>{classificationButtons}{task.sourceKind !== "proper" ? <div className="rounded-2xl bg-white p-4"><p className="mb-3 text-center font-black">{task.sourceKind === "improper" ? "Zamień na liczbę mieszaną" : "Zamień na ułamek niewłaściwy"}</p><FractionStackInput value={response} onChange={(value) => { setResponse(value); clearResult(); }} showWholePart={task.sourceKind === "improper"} readOnly={controlsLocked} stepLabel="Wykonaj zamianę" /></div> : <p className="rounded-xl bg-cyan-50 p-3 text-center font-bold">Ułamek właściwy nie zawiera pełnej całości — przejdź do osi.</p>}<MixedNumberLine denominator={task.target.denominator} numerator={axisNumerator} onChange={(value) => { setAxisNumerator(value); clearResult(); }} readOnly={controlsLocked} />{!controlsLocked ? <button type="button" className={styles.primaryButton} onClick={checkIndependent}>Sprawdź odpowiedzi</button> : null}</div> : null}
