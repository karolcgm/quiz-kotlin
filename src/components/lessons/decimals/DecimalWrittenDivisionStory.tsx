...
import { AlignedDecimalDivisionGrid } from "@/components/lessons/decimals/DecimalNaturalDivideL1Lab";
import { shiftDecimalCommaRight } from "@/lib/math/decimals/decimalDivideByDecimalL1";
import { buildDecimalNaturalLongDivisionSteps } from "@/lib/math/decimals/decimalNaturalDivideL1";
...
export function DecimalWrittenDivisionStory({ left: taskLeft, right: taskRight, result, unit, readOnly = false, onResultChange }: { left: string; right: string; result: string; unit: string; readOnly?: boolean; onResultChange?: (correct: boolean | null, answer?: string) => void }) {
  ...
  const steps = useMemo(() => buildDecimalNaturalLongDivisionSteps(finalDividend, Number(finalDivisor), 0), [finalDividend, finalDivisor]);
  const [products, setProducts] = useState(() => steps.map((step) => readOnly ? [...raw(step.productDisplay)] : raw(step.productDisplay).split("").map(() => "")));
  const [remainders, setRemainders] = useState(() => steps.map((step) => readOnly ? [...raw(step.nextDisplay)] : raw(step.nextDisplay).split("").map(() => "")));
  ...
  const check = () => {
    const correct = setupCorrect && shiftCount === requiredShifts && quotient.join("") === resultDigits && steps.every((step, index) => products[index]?.join("") === raw(step.productDisplay) && remainders[index]?.join("") === raw(step.nextDisplay));
  ...
  return <section className="space-y-4 rounded-2xl border-2 border-amber-300 bg-amber-50 p-5" data-decimal-story-division>
    ...
    {setupCorrect ? <div className="space-y-4 rounded-xl border-2 border-indigo-200 bg-indigo-50 p-4"><div className="grid items-center gap-3 md:grid-cols-[1fr_auto_1fr]"><p className="rounded-xl bg-white p-3 text-center text-xl font-black">{taskLeft} : {taskRight}</p><button type="button" disabled={readOnly || shiftCount >= requiredShifts} onClick={() => { setShiftCount((count) => count + 1); clear(); }} className="rounded-xl border-2 border-indigo-600 bg-indigo-600 px-4 py-3 font-black text-white">Przesuń oba przecinki o 1 miejsce →</button><p className="rounded-xl bg-emerald-50 p-3 text-center text-xl font-black">{shiftDecimalCommaRight(taskLeft, shiftCount)} : {shiftDecimalCommaRight(taskRight, shiftCount)}</p></div>{shiftCount === requiredShifts ? <div className="space-y-3"><p className="text-center font-black text-indigo-950">Teraz dzielimy pisemnie: {finalDividend} : {finalDivisor}</p><div className="mx-auto w-fit rounded-xl border-2 border-indigo-400 bg-white p-4 font-mono font-black text-slate-950"><AlignedDecimalDivisionGrid dividend={finalDividend} divisor={Number(finalDivisor)} quotient={quotient} resultCommaAfter={commaAfter(result)} products={products} remainders={remainders} steps={steps} active={active.row === "quotient" || active.row === "product" || active.row === "remainder" ? active : undefined} onSelect={(selection) => { if (selection.row !== "answer") setActive(selection); }} readOnly={readOnly} showDividendComma={false} showQuotientComma={result.includes(",")} label={`Dzielenie pisemne ${finalDividend} przez ${finalDivisor}`} /></div></div> : <p className="text-center font-bold text-indigo-950">Przesuwaj przecinki aż dzielnik będzie liczbą naturalną.</p>}</div> : <p className="rounded-xl bg-white p-3 text-center font-bold text-amber-950">Wpisz poprawnie obie liczby, aby przejść do przesuwania przecinków.</p>}
