...
export function DecimalEstimateL1Lab(props: DecimalEstimateL1LabProps) {
  ...
  return <LessonTaskFrame eyebrow="Dział 5 · Ułamki dziesiętne" heading={isRound ? "Najpierw oszacuj" : "Czy wynik ma sens?"} description={isRound ? "Zaokrąglij liczby w myślach i wybierz wynik przybliżony. Nie obliczaj jeszcze dokładnie." : "Oszacowanie pomaga zweryfikować wynik działania. Jeśli wynik nie pasuje do oszacowania, sprawdź położenie przecinka."} questionNumber={props.questionNumber} questionCount={props.questionCount} contentClassName="space-y-5" data-decimal-estimate-l1 data-decimal-activity={props.activity}>
    ...
    {status ? <p role="status" className={`rounded-xl p-4 text-center font-black ${status === "correct" ? "bg-emerald-100 text-emerald-950" : "bg-rose-100 text-rose-950"}`}>{status === "correct" ? isRound ? "Dobrze! Oszacowanie pomaga sprawdzić, czy dokładny wynik ma sens." : sense.answer ? "Dobrze! Oszacowanie potwierdza, że wynik działania ma właściwy rząd wielkości." : "Dobrze! Oszacowanie wykryło błąd: przecinek jest w złym miejscu. Możemy teraz poprawić wynik działania." : "Spróbuj jeszcze raz. Porównaj rząd wielkości wyniku z oszacowaniem."}</p> : null}
  </LessonTaskFrame>;
}
