...
export function AnimatedSticker({ stickerId, size = "md", selected = false }: { stickerId: number; size?: "sm" | "md" | "xl"; selected?: boolean }) {
  ...
  return <div className={`reward-sticker relative grid shrink-0 place-items-center overflow-hidden rounded-[30%] border-4 border-white bg-white shadow-xl ${sizeClass}`} data-selected={selected || undefined} role="img" aria-label={`${sticker.name}, kolekcja ${sticker.collectionName}`}>
