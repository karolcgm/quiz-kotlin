...
5. **Seria zadań — uruchom maszynę**: 5 unikalnych zadań o rosnącej złożoności. W każdym przykładzie uczeń widzi wyrażenie z literą i przesuwa suwak z `x` na wskazaną liczbę. Dopiero po podstawieniu odblokowuje się pole wyniku; uczeń sam wykonuje obliczenia i wpisuje odpowiedź klawiaturą lekcji.
6. **Bilet wyjścia**: samodzielne podstawienie suwakiem i obliczenie wartości z zachowaniem kolejności działań.
